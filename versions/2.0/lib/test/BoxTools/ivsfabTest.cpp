...
int  sum( const IntVect & a_iv ){ return D_TERM(a_iv[0], +2*a_iv[1], +4*a_iv[2]); }
...
int testIVS()
{
  ...
  if( ! ( ivs == ivstest ) )
  {
    if( verbose ) cout << "error: " << pgmname << ": failed IntVectSet consistency test" << endl;
    ++errors ;
  }
  // test that numIvs() returns the expected result
  if( fab.numIvs() != box.volume() )
  {
    if( verbose ) cout << "error: " << pgmname << ": failed numIvs() test" << endl;
    ++errors ;
  }
  ...
  for( BoxIterator bit(box) ; bit.ok() ; ++bit )
  {
    for(int icomp = 0; icomp < ncomp; icomp++)
    {
      ...
      {
        if( verbose )
          cout << "error: " << pgmname << ": value at (" << bit() << "," << icomp
  ...
  for(IVSIterator ivsit(ivs); ivsit.ok(); ++ivsit)
  {
    for(int icomp = 0; icomp < ncomp; icomp++)
    {
      ...
      {
        if( verbose )
          cout << "error: " << pgmname << ": value at (" << ivsit() << "," << icomp
