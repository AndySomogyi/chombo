...
copyTest()
{
  ...
  bogus spacedim;
#endif
  loLayoutCoar.addBox(b1,0);
  loLayoutCoar.addBox(b2,0);
  loLayoutCoar.addBox(b3,0);
  ...
  else CH_assert( (CH_SPACEDIM>0) && (CH_SPACEDIM<4) );
  if( fabs( norm( loFabCoar, Interval(0,0), 2 ) - normAnswer ) > eps )
  {
    if(verbose)
      {
        cout << "norm(BoxLayoutData) is wrong" << endl;
      }
    icode = -3;
  }
  ...
  for(ditFine.reset(); ditFine.ok(); ++ditFine)
    {
      ...
      if(Abs(rmax - valcomp) > eps)
        {
          if(verbose)
            cout << indent2 << "max is wrong" << endl;
          icode =-1;
        }
      if(Abs(rmin - valcomp) > eps)
        {
          if(verbose)
            cout << indent2 << "min is wrong" << endl;
          icode =-2;
        }
    }
  if(verbose)
    {
      cout << indent2 << "LoFabCoar:" << endl;
      for(ditCoar.reset(); ditCoar.ok(); ++ditCoar)
        {
          FArrayBox& fab = loFabCoar[ditCoar()];
          ...
          for(bit.reset(); bit.ok(); ++bit)
            {
              cout << indent2 << bit() << indent2 << fab(bit(),0) << endl;
            }
        }
      cout << indent2 << "LoFabFine:" << endl;
      for(ditFine.reset(); ditFine.ok(); ++ditFine)
        {
          FArrayBox& fab = loFabFine[ditFine()];
          BoxIterator bit(fab.box());
