...
int test()
{
 ...
 CH_assert(testFile.isOpen());
  ...
  if(error != 0)
    {
      if( verbose )
        cout << indent2 << "attribute write failed "<<error<<endl;
      return error;
    }
  ...
  if(error != 0)
    {
      if( verbose )
        cout << indent2 << "set group failed "<<error<<endl;
      return error;
    }
 ...
 CH_assert(testFile.isOpen());
  ...
  if(error != 0)
    {
      if( verbose )
        cout << indent2 << "attribute read failed "<<error<<endl;
      return error;
    }
  ...
  if(error != 0)
    {
      if( verbose )
        {
          cout << indent2 << "attribute read does not match write in root"<<error<<"\n";
          cout << set1;
          cout <<"readData:\n";
          cout <<readData<<endl;
        }
  ...
  if(readData.m_int.find("currentStep") != readData.m_int.end())
    {
      if( verbose )
        cout << indent2 << "HDF5HeaderData.clear() not functioning correct"<<endl;
      return 1;
    }
