...
// header file for the test harness
#undef TEST_ARRAYVIEW
...
void Test::Testpout( int argc, char *argv[] ){
  ...
  { // test that setting a new name writes to a new file and doesn't change the old file
    ...
    if( debug ){ std::cout << std::endl << "dbg: new filename is [" << filename2 << "] ... " ; }
    int compare = strncmp( filename2.c_str() ,TestBaseName2.c_str() ,TestBaseName2.size() ) ;
    if( debug ){ std::cout << std::endl << "dbg: strcmp(filename,basename) returned " << compare << " ... " ; }
    ...
    pin >> line1 ;
    pin >> line2 ;
    if( debug ){ std::cout << std::endl << "dbg: line1,2 are [" << line1 << "],[" << line2 << "] ... " ; }
    Fail( line1 != TestLine1 ,false ) ;
