...
testRCP()
{
  ...
  if (rv2.isNonUnique() || rv.isNonUnique())
  {
    if (verbose)
    {
      cout << indent2 << "RefCountedPtr failed isNonUnique" << endl;
    }
    return_code = -1;
  }
    ...
    if(rv3.refCount() != 3)
      {
        if (verbose)
          {
            cout << indent2 << "RefCountedPtr::refCount is wrong" << endl;
          }
        return_code = -1;
      }
  ...
  if(myBar->cherry != 5 || myBar2.refCount() != 1 || myFoo.refCount() != 3 || myBar.refCount() != 3)
    {
      if (verbose)
        {
          cout << indent2 << "RefCountedPtr template conversion is wrong" << endl;
        }
      return_code = -1;
    }
...
//
  if (return_code == 0)
  {
    cout << indent << "testRefCountedPtr test passed." << endl;
  }
  else
  {
    cout << indent << "testRefCountedPtr test FAILED!!!" << endl;
  }
