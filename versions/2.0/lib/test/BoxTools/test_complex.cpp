...
int main(int argc, char *argv[]){
  ...
  if( verbose ) cout << indent << "Beginning " << pgmname << " ..." << endl ;

  std::complex<double> c(123.4567,765.4321);
  double r1,i1,r2,i2 ;
  r1 = c.real() ; i1 = c.imag() ;
  if( verbose ) cout << indent2 << "before: c = " << c << endl ;
  ...
  if( verbose ) cout << indent2 << "after: c = " << c << endl ;
  r2 = c.real() ; i2 = c.imag() ;
  if( r1 != i2 || i1 != r2 ){
    cout << "error: " << pgmname
