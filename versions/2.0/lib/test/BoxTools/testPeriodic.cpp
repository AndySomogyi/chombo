...
// if true, use arrayview to display results
...
main(int argc ,char *argv[] )
{
        ...
        for (int dir=0; dir<SpaceDim; dir++)
          {
            ...
            Box loGhostBox = adjCellLo(baseDomBox, dir, 1);
            ...
            Box hiGhostBox = adjCellHi(baseDomBox, dir, 1);
  ...
  baseDomain.refine(2);
  {
    ...
    if (SpaceDim == 1)
      ...
      {
        levelBoxes2[0] = Box(IntVect::Zero, 9*IntVect::Unit);
        levelBoxes2[1] = Box(IntVect(D_DECL(7,10,10)), 15*IntVect::Unit);
        // this box should fail the disjointness test
        //levelBoxes2[1] = Box(IntVect(D_DECL(7,10,10)), 17*IntVect::Unit);
        levelBoxes2[2] = Box(IntVect(D_DECL(11,0,0)), IntVect(D_DECL(15,7,7)));
      }
            ...
            for (int dir=0; dir<SpaceDim; dir++)
              {
                ...
                Box loGhost = adjCellLo(domainBox, dir, 1);
                loGhost &= thisBox;
                if (!loGhost.isEmpty())
                {
                ...
                Box hiGhost = adjCellHi(domainBox, dir, 1);
                hiGhost &= thisBox;
                if (!hiGhost.isEmpty())
                  {
  ...
  if (SpaceDim > 1)
    {
      ...
      levelBoxes2[1] = Box(IntVect(D_DECL(3,5,5)), 7*IntVect::Unit);
      // this box should fail the disjointness test
      //levelBoxes2[1] = Box(IntVect(D_DECL(3,5,5)), 10*IntVect::Unit);
      levelBoxes2[2] = Box(IntVect(D_DECL(6,0,0)), IntVect(D_DECL(7,1,1)));
...
Real dataVal(const IntVect& a_loc, int a_comp)
{
  Real localVal = a_comp*(D_TERM(0.1*a_loc[0]+0.5,
                                 +a_loc[1]+0.5,
                                 +100*a_loc[2]+0.5));
