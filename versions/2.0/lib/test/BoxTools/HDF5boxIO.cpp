 ...
 CH_assert(!testFile.isOpen());
  ...
  error = read(testFile, boxes);
   ...
   if(plan1.size() != readplan1.size())
     {
       if( verbose )
         cout << indent2 << "plan1 size different on read "<<endl;
       return 1;
     }
       ...
       if(plan1.get(p1()) != readplan1.get(rp1()))
         {
           if( verbose )
             {
               cout << indent2 << "plan1 != readplan1  on read \n";
               cout<<plan1<<"\n\n"<<readplan1<<endl;
             }
           return 2;
         }
   ...
   if(plan2.size() != readplan2.size())
     {
       if( verbose )
         cout << indent2 << "plan2 size different on read "<<endl;
       return 1;
     }
       ...
       if(plan2.get(p2()) != readplan2.get(rp2()))
         {
           if( verbose )
             {
               cout << indent2 << "plan2 != readplan2  on read \n";
               cout<<plan2<<"\n\n"<<readplan2<<endl;
             }
           return 2;
...
#else
#error arrayTest failed: implemented only for 1D, 2D and 3D
#endif
    }
  }
}
