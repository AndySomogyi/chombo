...
int domainSplitTest()
{
  ...
  for(int ibox = 0; ibox < vbox.size(); ibox++)
    {
      ...
      if(!domain.contains(boxloc))
        {
          if(verbose)
            cerr << "error: " << boxloc << "not contained in " << domain  << endl;
          retflag += 1;
        }
      ivsDom -= boxloc;
      for(int idir = 0; idir < SpaceDim; idir++)
        {
          if(boxloc.size(idir) > maxlen)
            {
              if(verbose)
                cerr << "error: " << boxloc << "is too big in " << idir
  ...
  if(!ivsDom.isEmpty())
    {
      if(verbose)
        cerr << "error: domain not covered " << endl;
      retflag += 4;
    }
  if(verbose)
    {
      cout << "domain = " << domain << endl;
      cout << "max box size = " << maxlen << endl;
      cout << "boxes = " << endl;
      for(int ibox = 0; ibox < vbox.size(); ibox++)
        {
          const Box& boxloc = vbox[ibox];
  ...
  {
    Box testDomainBox(IntVect::Zero, IntVect(D_DECL(1, 63, 15)));
    int ncellmax = 16;
    int block_factor = 2;
    Vector<Box> boxes;
