...
main(int argc, char* argv[])
{
  ...
  if(test() != 0)
    {
      cout << indent << pgmname <<" failed"<<endl;
    }
  else
    {
...
int test()
{
 ...
 CH_assert(testFile.isOpen());
  ...
  if( verbose )
  {
    cout << "plan1: " << procID() << "...." << plan1 << endl;
    cout << "plan2: " << procID() << "...." << plan2 << endl;
  }
  ...
  if(error != 0)
    {
      if( verbose )
        cout << indent2 << "BoxLayoutData 1 write failed "<<error<<endl;
      return error;
    }
  ...
  if(error != 0)
    {
      if( verbose )
        cout << indent2 << "box2 write failed "<<error<<endl;
      return error;
    }
  ...
  if(error != 0)
    {
      if( verbose )
        cout << indent2 << "BoxLayoutData 2 write failed "<<error<<endl;
      return error;
    }
 ...
 CH_assert(!testFile.isOpen());
  ...
  if(error != 0)
    {
      if( verbose )
        cout << indent2 << "box read failed "<<error<<endl;
      return error;
    }
  boxes.sort();
  Vector<int> assign;
  ...
  if(error != 0)
    {
      if( verbose )
        cout << indent2 << "BoxLayout LoadBalance failed "<<error<<endl;
      return error;
    }
  ...
  if(error != 0)
    {
      if( verbose )
        cout << indent2 << "BoxLayoutData<BaseFab<int>> read failed "<<error<<endl;
      return error;
    }
  ...
  if( verbose )
  {
    cout << "readplan1: " << procID() << "...." << readplan1 << endl;
    cout << "readplan2: " << procID() << "...." << readplan2 << endl;
  }
  ...
  if(level1.boxLayout().size() != readlevel1.boxLayout().size())
    {
      if( verbose )
        cout << indent2 << "level1.size() != readl1.size() read failed "<<error<<endl;
      return 1;
    }
  if(level2.boxLayout().size() != readlevel2.boxLayout().size())
    {
      if( verbose )
        cout << indent2 << "level2.size() != readl2.size() read failed "<<error<<endl;
      return 1;
    }
          ...
          for(BoxIterator it(level1.box(l1())); it.ok(); ++it)
            {
              if((*before)(it(), c) != (*after)(it(), c))
                {
                  if( verbose )
                    cout << indent2 << "l1 != readl1 read failed "<<error<<endl;
                  return 2;
                }
            }
        }
          ...
          for(BoxIterator it(level2.box(l2())); it.ok(); ++it)
            {
              if((*before)(it(), c) != (*after)(it(), c))
                {
                  if( verbose )
                    cout << indent2 << "level2 != readlevel2 read failed "<<error<<endl;
                  return 3;
                }
            }
        }
  ...
  if(error != 0)
    {
      if( verbose )
        cout << indent2 << "readLevel failed "<<error<<endl;
      return error;
    }
          ...
          for(BoxIterator it(state.box(dit())) ; it.ok() ; ++it)
                {
                  if(readFAB(it(), 0) != fab(it(), 1))
                        {
                          if( verbose )
                                cout << indent2 << "state != after for out-of-core "<<error<<endl;
                          return 3;
                        }
                }
        }
