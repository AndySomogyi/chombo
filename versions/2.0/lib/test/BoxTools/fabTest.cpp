...
int fabTest()
{
  ...
  if( answer != theindex )
  {
    cout << "minIndex() might have failed (returned " << answer
  ...
  if( answer != theindex )
  {
    cout << "maxIndex() might have failed (returned " << answer
         << ", should be " << theindex << ")" << endl;
    icode = -18;
  }
  ...
  ///norm tests
  if((fabs(ourfabs[0].norm(0,0,1) - 1.0) > eps) ||
     (fabs(ourfabs[1].norm(0,0,1) - 2.0) > eps) ||
     (fabs(ourfabs[2].norm(0,0,1) - 3.0) > eps) ||
     (fabs(ourfabs[3].norm(0,0,1) - 4.0) > eps))
    {
      cout << indent << pgmname
  ...
  Real rnorm = B.numPts();
  if((fabs(ourfabs[0].norm(1,0,3)/rnorm - 3*1.0) > eps) ||
     (fabs(ourfabs[1].norm(1,0,3)/rnorm - 3*2.0) > eps) ||
     (fabs(ourfabs[2].norm(1,1,2)/rnorm - 2*3.0) > eps) ||
     (fabs(ourfabs[3].norm(1,2,1)/rnorm - 1*4.0) > eps))
    {
      cout << indent << pgmname
  ...
  rnorm = sqrt(rnorm);
  if((fabs(ourfabs[0].norm(2,0,1)/rnorm - 1.0) > eps) ||
     (fabs(ourfabs[1].norm(2,0,1)/rnorm - 2.0) > eps) ||
     (fabs(ourfabs[2].norm(2,0,1)/rnorm - 3.0) > eps) ||
     (fabs(ourfabs[3].norm(2,0,1)/rnorm - 4.0) > eps))
    {
      cout << indent << pgmname
  ...
  {
    cout << "ourfabs[0].dotProduct(ourfabs[0])/numPts = (should=3.0) "
         << ourfabs[0].dotProduct(ourfabs[0])/numPts << '\n';
    cout << "ourfabs[1].dotProduct(ourfabs[1])/numPts = (should=-6.0) "
  ...
  FArrayBox iotaFab( Box(IntVect::Zero, IntVect(D_DECL(1,2,3))), 2 );
  ...
  if( maskCount != 4 )
  {
    if(verbose)
    {
      cout << "maskLT failed: returned " << maskCount << ", should be 4\n";
    }
    icode = -20;
  }
  ...
  if( maskCount != 5 )
  {
    if(verbose)
    {
      cout << "maskLE failed: returned " << maskCount << ", should be 5\n";
    }
    icode = -20;
  }
  ...
  if( maskCount != 1 )
  {
    if(verbose)
    {
      cout << "maskEQ failed: returned " << maskCount << ", should be 1\n";
    }
    icode = -20;
  }
  ...
  //
  {
    ...
    if( sumAnswer != correctAnswer )
    {
      if(verbose)
      {
        cout << "sum() failed: returned " << sumAnswer << ", should be "
             << correctAnswer << '\n';
      }
      icode = -21;
    }
  ...
  //
  {
    ...
    else                        correctAnswer = 1.18924;
    if( fabs( sumAnswer - correctAnswer ) > 0.00001 )
    {
      if(verbose)
      {
        cout << "inverse() failed: returned " << sumAnswer << ", correct="
             << correctAnswer << '\n';
      }
      icode = -22;
    }
 ...
 CH_assert(buf != NULL);
  ...
  if( verbose ) cout << indent2 << pgmname
