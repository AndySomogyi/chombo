...
main(int argc ,char* argv[])
{
  ...
  {//scoping trick to make sure everything is destructed before MPI_FINALIZE is called
    ...
    { // Test File input/output in Chombo Fortran
      ...
      FORT_CHF_FIO( CHF_CONST_INT( i1 ) ,CHF_CONST_REAL( r1 ), CHF_INT(status2) ) ;

      if( status2 == 0 ){
        if( verbose )
          cout << indent << pgmname << " passed the file I/O tests." << endl ;
      }
      else
      {
        cout << indent << pgmname << " failed the file I/O tests with return code " << status2 << endl ;
    ...
    { // Test standard input/output in ChF
      if( test_stdio )
      {
        int i1 = 23 ;
        Real r1 = 23.45 ;
        cout << indent << pgmname << ": running std I/O tests.  You should see some output" << endl ;
        int status2;
        FORT_CHF_STDIO( CHF_CONST_INT( i1 ) ,CHF_CONST_REAL( r1 ), CHF_INT(status2) ) ;
        if( status2 == 0 )
        {
          if( verbose )
            cout << indent << pgmname << " passed the std I/O tests." << endl ;
        }
        else
        {
          cout << indent << pgmname << " failed the std I/O tests with return code " << status << endl ;
          if( status == 0 ) status = status2 ;
        }
      }
    }
    ...
    { // Test ChomboFortran argument list macros for Vector<>
      ...
      {
       if( verbose )
         cout << indent2 << "error: chfptr.i returned " << p.i << " instead of 0" << endl ;
        ++status2 ;
      }
      if( (const int *)p != &p.i )
      {
        if( verbose )
          cout << indent2 << "error: (const int * const)chfptr returned " << (const int *)p
               << "instead of " << &p.i << endl ;
        ++status2 ;
      }
      ...
      {
        if( verbose )
          cout << indent2 << "error: TESTVIR_1 returned int arg 1 = " << vectorI[0]
               << " instead of " << I1r << endl;
        ++status2 ;
      }
      if( vectorCI[0] != I20 )
      {
        if( verbose )
          cout << indent2 << "error: TESTVIR_1 returned const int arg 2 [0] = " << vectorCI[0]
               << " instead of " << I20 << endl ;
        ++status2 ;
      }
      if( vectorCI[1] != I21 )
      {
        if( verbose )
          cout << indent2 << "error: TESTVIR_1 returned const int arg 2 [1] = " << vectorCI[1]
               << " instead of " << I21 << endl ;
        ++status2 ;
      }
      if( vectorR[0] != R10r )
      {
        if( verbose )
          cout << indent2 << "error: TESTVIR_1 returned Real arg 3 [0] = " << vectorR[0]
      ...
      {
        if( verbose )
          cout << indent2 << "error: TESTVIR_1 returned Real arg 3 [1] = " << vectorR[1]
      ...
      {
        if( verbose )
          cout << indent2 << "error: TESTVIR_1 returned Real arg 3 [2] = " << vectorR[2]
      ...
      {
        if( verbose )
          cout << indent2 << "error: TESTVIR_1 returned const Real arg 4 [0] = " << vectorCR[0]
               << " instead of " << R20 << endl;
        ++status2 ;
      }
      if( vectorCR[1] != R21 )
      {
        if( verbose )
          cout << indent2 << "error: TESTVIR_1 returned const Real arg 4 [1] = " << vectorCR[1]
               << " instead of " << R21 << endl;
        ++status2 ;
      }
      if( vectorCR[2] != R22 )
      {
        if( verbose )
          cout << indent2 << "error: TESTVIR_1 returned const Real arg 4 [2] = " << vectorCR[2]
               << " instead of " << R22 << endl;
        ++status2 ;
      }
      if( vectorCR[3] != R23 )
      {
        if( verbose )
          cout << indent2 << "error: TESTVIR_1 returned const Real arg 4 [3] = " << vectorCR[3]
               << " instead of " << R23 << endl;
        ++status2 ;
      }
      // done with tests
      ...
      {
        if( verbose )
          cout << indent << pgmname << " passed the ChF Vector<> tests." << endl ;
      }
      else
      {
        if( verbose )
          cout << indent << pgmname << " failed " << status2 << " of the ChF Vector<> tests." << endl ;
        if( status == 0 ) status = status2 ;
      }
    }
    ...
    { // Test ChomboFortran 1D array macros
      ...
      const int arrayCI[2] = { I20,I21 } ;
      Real arrayR[3]  ; arrayR[0]  = R10 ; arrayR[1]  = R11 ; arrayR[2]  = R12 ;
      const Real arrayCR[4] = { R20 ,R21 ,R22 ,R23 } ;
      ...
      {
        if( verbose )
          cout << indent2 << "error: TESTIR1D_1 returned int arg 1 = " << arrayI[0]
               << " instead of " << I1r << endl;
        ++status2 ;
      }
      if( arrayCI[0] != I20 )
      {
        if( verbose )
          cout << indent2 << "error: TESTIR1D_1 returned const int arg 2 [0] = " << arrayCI[0]
               << " instead of " << I20 << endl ;
        ++status2 ;
      }
      if( arrayCI[1] != I21 )
      {
        if( verbose )
          cout << indent2 << "error: TESTIR1D_1 returned const int arg 2 [1] = " << arrayCI[1]
               << " instead of " << I21 << endl ;
        ++status2 ;
      }
      if( arrayR[0] != R10r )
      {
        if( verbose )
          cout << indent2 << "error: TESTIR1D_1 returned Real arg 3 [0] = " << arrayR[0]
               << " instead of " << R10r << endl;
        ++status2 ;
      }
      if( arrayR[1] != R11r )
      {
        if( verbose )
          cout << indent2 << "error: TESTIR1D_1 returned Real arg 3 [1] = " << arrayR[1]
               << " instead of " << R11r << endl;
        ++status2 ;
      }
      if( arrayR[2] != R12r )
      {
        if( verbose )
          cout << indent2 << "error: TESTIR1D_1 returned Real arg 3 [2] = " << arrayR[2]
               << " instead of " << R12r << endl;
        ++status2 ;
      }
      if( arrayCR[0] != R20 )
      {
        if( verbose )
          cout << indent2 << "error: TESTIR1D_1 returned const Real arg 4 [0] = " << arrayCR[0]
               << " instead of " << R20 << endl;
        ++status2 ;
      }
      if( arrayCR[1] != R21 )
      {
        if( verbose )
          cout << indent2 << "error: TESTIR1D_1 returned const Real arg 4 [1] = " << arrayCR[1]
               << " instead of " << R21 << endl;
        ++status2 ;
      }
      if( arrayCR[2] != R22 )
      {
        if( verbose )
          cout << indent2 << "error: TESTIR1D_1 returned const Real arg 4 [2] = " << arrayCR[2]
               << " instead of " << R22 << endl;
        ++status2 ;
      }
      if( arrayCR[3] != R23 )
      {
        if( verbose )
          cout << indent2 << "error: TESTIR1D_1 returned const Real arg 4 [3] = " << arrayCR[3]
               << " instead of " << R23 << endl;
        ++status2 ;
      }
      // done with tests
      ...
      {
        if( verbose )
          cout << indent << pgmname << " passed the ChF *1D tests." << endl ;
      }
      else
      {
        if( verbose )
          cout << indent << pgmname << " failed " << status2 << " of the ChF *1D tests." << endl ;
        if( status == 0 ) status = status2 ;
      }
    }
...
//      const int IVerbose = verbose ? 1 : 0 ;
      ...
      {
        if( verbose )
          cout << indent2 << "error: TESTCALL returned int arg 1 = " << arrayI[0]
               << " instead of " << I1r << endl;
        ++status2 ;
      }
      if( arrayCI[0] != I20 )
      {
        if( verbose )
          cout << indent2 << "error: TESTCALL returned const int arg 2 [0] = " << arrayCI[0]
               << " instead of " << I20 << endl ;
        ++status2 ;
      }
      if( arrayCI[1] != I21 )
      {
        if( verbose )
          cout << indent2 << "error: TESTCALL returned const int arg 2 [1] = " << arrayCI[1]
               << " instead of " << I21 << endl ;
        ++status2 ;
      }
      if( arrayR[0] != R10r )
      {
        if( verbose )
          cout << indent2 << "error: TESTCALL returned Real arg 3 [0] = " << arrayR[0]
               << " instead of " << R10r << endl;
        ++status2 ;
      }
      if( arrayR[1] != R11r )
      {
        if( verbose )
          cout << indent2 << "error: TESTCALL returned Real arg 3 [1] = " << arrayR[1]
               << " instead of " << R11r << endl;
        ++status2 ;
      }
      if( arrayR[2] != R12r )
      {
        if( verbose )
          cout << indent2 << "error: TESTCALL returned Real arg 3 [2] = " << arrayR[2]
               << " instead of " << R12r << endl;
        ++status2 ;
      }
      if( arrayCR[0] != R20 )
      {
        if( verbose )
          cout << indent2 << "error: TESTCALL returned const Real arg 4 [0] = " << arrayCR[0]
               << " instead of " << R20 << endl;
        ++status2 ;
      }
      if( arrayCR[1] != R21 )
      {
        if( verbose )
          cout << indent2 << "error: TESTCALL returned const Real arg 4 [1] = " << arrayCR[1]
               << " instead of " << R21 << endl;
        ++status2 ;
      }
      if( arrayCR[2] != R22 )
      {
        if( verbose )
          cout << indent2 << "error: TESTCALL returned const Real arg 4 [2] = " << arrayCR[2]
               << " instead of " << R22 << endl;
        ++status2 ;
      }
      if( arrayCR[3] != R23 )
      {
        if( verbose )
          cout << indent2 << "error: TESTCALL returned const Real arg 4 [3] = " << arrayCR[3]
               << " instead of " << R23 << endl;
        ++status2 ;
      }
      // done with tests
      ...
      {
        if( verbose )
          cout << indent << pgmname << " passed the ChF calls ChF tests." << endl ;
      }
      else
      {
        if( verbose )
          cout << indent << pgmname << " failed " << status2 << " of the ChF calls ChF tests." << endl ;
        if( status == 0 ) status = status2 ;
      }
    }
    ...
    { // Test ChomboFortran CHF_DSELECT macro
      ...
      {
        if( verbose )
          cout << indent << pgmname << " passed the CHF_DSELECT tests." << endl ;
      }
      else
      {
        if( verbose )
          cout << indent << pgmname << " failed " << status2 << " of the CHF_DSELECT tests." << endl ;
        if( status == 0 ) status = status2 ;
      }
    }
    ...
    { // Test ChomboFortran CHF_MULTIDO macro
      ...
      if( status2 == 0 ){
        if( verbose )
          cout << indent << pgmname << " passed the CHF_MULTIDO tests." << endl ;
      }else{
        if( verbose )
          cout << indent << pgmname << " failed " << status2 << " of the CHF_MULTIDO tests." << endl ;
        if( status == 0 ) status = status2 ;
      }
    }
    ...
    { // Test calling MayDay routines (testing only one is enough).
      ...
      if( test_mayday )
      {
        ...
        cout << indent << pgmname << ": running MayDay tests. An Error or Abort should happen." << endl ;
        int status2;
        FORT_CHF_MAYDAY(CHF_INT(status2) ) ;
        cout << indent << pgmname << " failed the MayDay tests with return code " << status << endl ;
        if( status == 0 ) status = status2 ;
      }
    }
    ...
    { // Regression test for parsing end-of-line comments.
      ...
      if( verbose ){
        cout << indent << pgmname ;
        if( status2 == 0 ) cout << " passed " ;
        else               cout << " failed " << status2 << " of " ;
        cout << "the parse tests." << endl ;
      }
      if( status == 0 ) status = status2 ;
    }
