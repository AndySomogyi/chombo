...
main(int argc ,char *argv[] )
{
...

  {
    ...
    Vector<Box> crseBoxes(4);
    crseBoxes[0] = Box(IntVect::Zero, 4*IntVect::Unit);
    crseBoxes[1] = Box(IntVect(D_DECL(0,5,0)),
               IntVect(D_DECL(4,7,7)));
    crseBoxes[2] = Box(IntVect(D_DECL(5,0,0)),
               IntVect(D_DECL(7,4,7)));
    crseBoxes[3] = Box(5*IntVect::Unit, 7*IntVect::Unit);
    ...
    fineBoxes[1] = Box(IntVect(D_DECL(6,10,10)), IntVect(D_DECL(11,15,15)));
    // this box should fail the disjointness test
    //fineBoxes[1] = Box(IntVect(D_DECL(7,10,10)), 17*IntVect::Unit);
    fineBoxes[2] = Box(IntVect(D_DECL(12,2,0)), IntVect(D_DECL(15,7,7)));
      ...
      for (boxIt.begin(); boxIt.ok(); ++boxIt)
        {
          ...
          D_TERM(realLoc[0] = a_dx*(loc[0]+0.5);,
