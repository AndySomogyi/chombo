  ...
  if( status == 0 )
    {
      ...
      cout << " failed with result code " << status << endl ;
    }
...
testAverage()
{
  ...
  const Box coarse_domain(IntVect(D_DECL(  0,  0, 0)),
                          IntVect(D_DECL( 11, 11, 3)));
  const ProblemDomain coarse_problem_domain(coarse_domain);
  if (verbose)
  {
    cout << indent2 << "coarse problem domain = " << coarse_domain << endl;
  }

  const int num_coarse_grids = 2;
  if (verbose)
  {
    cout << indent2 << "num coarse grids = " << num_coarse_grids << endl;
  }
  Vector<int> assign;
  Vector<Box> coarse_boxes(num_coarse_grids);
  coarse_boxes[0] = Box( IntVect( D_DECL (0,  0, 0) ),
                         IntVect( D_DECL (5,  9, 3) ));
  coarse_boxes[1] = Box(IntVect( D_DECL ( 6,  2, 0) ),
                        IntVect( D_DECL (11, 11, 3) ));
  ...
  if (verbose)
  {
    cout << indent2 << "coarse grids: " << endl;
    LayoutIterator coarse_lit = coarse_grids.layoutIterator();
    for (coarse_lit.begin(); coarse_lit.ok(); ++coarse_lit)
    {
      cout << indent2 << coarse_lit().intCode() << ": " << coarse_grids[coarse_lit()] << endl;
    }
  }
  int num_fine_grids = 3;
  if (verbose)
  {
    cout << indent2 << "num fine grids = " << num_fine_grids << endl;
  }
  ...
  fine_boxes[2] = Box(IntVect( 20 ),
                      IntVect( 23 ));
#else
  fine_boxes[0] = Box(IntVect( D_DECL (  2,  4, 0) ),
                      IntVect( D_DECL (  7, 11, 7) ));
  fine_boxes[1] = Box(IntVect( D_DECL (  6, 12, 0) ),
                      IntVect( D_DECL ( 23, 17, 7) ));
  fine_boxes[2] = Box(IntVect( D_DECL ( 20,  6, 0) ),
                      IntVect( D_DECL ( 23, 11, 7) ));
#endif
  ...
  if (verbose)
  {
    cout << indent2 << "num comps = " << num_comps << endl;
  }
  ...
  for (int pass=0; pass<nRefinements; pass++)
    {
      ...
      if (verbose)
        {
          cout << indent2 << "Refinement ratio = " << ref_ratio << endl;
        }
      ...
      if (verbose)
        {
          cout << indent2 << "fine problem domain = " << fine_domain << endl;
          
          cout << indent2 << "fine grids: " << endl;
          LayoutIterator fine_lit = fine_grids.layoutIterator();
          for (fine_lit.begin(); fine_lit.ok(); ++fine_lit)
            {
              cout << indent2 << fine_lit().intCode() << ": " << fine_grids[fine_lit()] << endl;
            }
        }
      ...
      if (verbose)
        {
          cout << indent2 << "arithmetic data  sum = " << data_sum << endl;
          cout << indent2 << "arithmetic error sum = " << err_sum << endl;
        }
      ...
      if (err_sum < err_sum_tol)
        {
          if (verbose)
            cout << indent << pgmname << ": CoarseAverage arithmetic average test passed." << endl;
        }
      else
        {
          returnVal += 1;
          cout << indent << pgmname << ": CoarseAverage arithmetic average test FAILED!" << endl;
          if (verbose)
            {
              cout << indent2 << "error sum tolerance = " << err_sum_tol << endl;
            }
          return returnVal ;
        }
      ...
      if (verbose)
        {
          cout << indent2 << "harmonic data  sum = " << data_sum << endl;
          cout << indent2 << "harmonic error sum = " << err_sum << endl;
        }

      if (err_sum < err_sum_tol)
        {
          if (verbose)
            cout << indent << pgmname << ": CoarseAverage harmonic average test passed." << endl;
        }
      else
        {
          returnVal += 2;
          cout << indent << pgmname << ": CoarseAverage harmonic average test FAILED!" << endl;
          if (verbose)
            {
              cout << indent2 << "error sum tolerance = " << err_sum_tol << endl;
            }
