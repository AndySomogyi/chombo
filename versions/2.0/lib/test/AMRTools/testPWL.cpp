...
testPWL()
{
  ...
  const Box coarse_problem_domain(IntVect(D_DECL(  0,  0, 0)),
                                  IntVect(D_DECL( 11, 11, 3)));
  if (verbose)
  {
    cout << indent2 << "coarse problem domain = " << coarse_problem_domain << endl;
  }
  const Box fine_problem_domain = refine(coarse_problem_domain, ref_ratio);
  if (verbose)
  {
    cout << indent2 << "fine problem domain = " << fine_problem_domain << endl;
  }
  ...
  if (verbose)
  {
    cout << indent2 << "interp radius = " << interp_radius << endl;
  }
...
//  pwl.writeIntVectSetsPlotfiles();
  ...
  if (verbose)
  {
    cout << indent2 << "time interp coef = " << time_interp_coef << endl;
  }
...
// compare with exact
  ...
  if (verbose)
  {
    cout << indent2 << "data  sum = " << data_sum << endl;
    cout << indent2 << "error sum = " << err_sum << endl;
  }
  ...
  if (err_sum < err_sum_tol)
    {
      if (verbose)
        cout << indent << pgmname << ": piecewise linear coarse-fine interpolation test passed." << endl;
      return 0 ;
    }
  else
    {
      cout << indent << pgmname << ": piecewise linear coarse-fine interpolation test FAILED!!!" << endl;
    if (verbose)
    {
