...
//   m_lastBuffer = 0;
  ...
  CH_assert( (a_maxSize >= 2*a_bufferSize) || (a_maxSize == 0) );
  CH_assert( (a_blockFactor <= a_maxSize) || (a_maxSize == 0) );
                   ...
                   const Vector<Vector<Box> >& a_OldMeshes) 
{
  ...
  Vector<IntVectSet> modifiedTags;

  //cout<<"entering regrid "<<procID()<<std::endl;
  //
  ...
  int level;
  for (level = Min(a_topLevel, isize-1);
       level >= a_baseLevel;
       --level)
    {
  ...
  if (level>=a_baseLevel)
    {
      TopLevel = level;
      // cout<<"found top "<<procID()<<std::endl;
      // copy tags to local storage so we can mess with them.

      // test for pathological case which causes proper nesting
      // to go off the deep end
      bool variableNRef = false;
      int maxNRef = m_nRefVect[a_baseLevel];
      for (int lev = a_baseLevel; lev<= TopLevel; lev++)
        {
          if (maxNRef != m_nRefVect[lev]) variableNRef = true;
          if (maxNRef < m_nRefVect[lev]) maxNRef = m_nRefVect[lev];
        }

      // this is the case where makePNDs screws up
      if (variableNRef &&  (m_blockFactor < maxNRef))
        {
          pout () << "MeshRefine error: If nRef varies, "
                  << "blockFactor may not be less than max ref ratio."
                  << endl;
      ...
      if( TopLevel+1 > a_baseLevel )
        {
          ...
          //
          // coarsen old meshes, tags, problem domains by the appropriate
          //  blocking factor
          ...
          for (int level = a_baseLevel; level <= TopLevel; ++level)
            {
              // first intersect tags with domains
              // (note that in periodic case, this can
              // result in wrapping of tags that are
              // outside the base domain box
	      ...
	      //	    <<" diff:"<<nBefore-nAfter<<" newDiff:"<<nTmp-nAfter <<"\n";	      
              ...
              // the goal of this is to coarsen everything down
              // to a level which is m_blockFactor coarser than
              // the new fine level we're going to generate.  By
              // generating grids at this fake level, we can then
              // refine up to the new level, and the blocking factor
          ...
          for (int level = 0; level < OldMeshes.size (); ++level)
            {
              if (level < m_level_blockfactors.size())
                {
                  for (int box = 0; box < OldMeshes[level].size (); ++box)
                    {
                      OldMeshes[level][box].coarsen(m_level_blockfactors[level]);
                    }
          ...
          // GHM 7/30/01
          // the buffer size argument to makePND needs to be coarsened by
          // the BlockFactor, since all the other arguments have been so
          // coarsened.  If not, a circumstance may occur where a set of
          // grids defined with a certain set of tags does not "refine" to
          // itself given the exact same set of tags.
          ...
          makePNDs(m_pnds, totalBufferSize, a_baseLevel, TopLevel, Domains,  OldMeshes[a_baseLevel],
                   blocked_BufferSize);

	  //pout()<<totalBufferSize;
	    ...
	    //pout()<<"nBefore:"<<nBefore<<" nAfter:"<<nAfter<<" diff:"<<nBefore-nAfter<<"\n";
          ...
          for( int lvl = TopLevel ; lvl >= a_baseLevel ; lvl-- ) {
            ProblemDomain lvldomain ; // domain of this level
                ...
                for (int i=0; i<lvlboxes.size(); i++)
                  {
                    ...
                    // will handle periodic wraparound through shifting
                    // and adding shifted image to tags, which will all
                    // remain within the domainBox
                    localBox &= domainBox;
                    modifiedTags[lvl] |= localBox;
                    // now do shifts to capture periodic images
                    // necessary to enforce proper nesting
                    // only do this if original box was not
                    // contained by domainBox
            ...
            //cout<<"before gathering  "<<procID()<<std::endl;
            ...
            //cout<<"gathering complete "<<procID()<<std::endl;
            broadcast( modifiedTags[lvl] , dest_proc);

            makeBoxes(lvlboxes, modifiedTags[lvl], m_pnds[lvl],
                      lvldomain, m_maxSize/m_blockFactor, totalBufferSize[lvl]);
            ...
            if( lvl > a_baseLevel )
              {
                ...
                for( int i = 0 ; i < lvlboxes.size() ; i++ )
                  {
                    // since we want to have a BufferSize border
                    // of (lvl-1) cells, this translates into
                    // growing the coarsened boxes by the BufferSize
                    // coarsened by the same factor we used to get the
                    // tags from level (lvl-1) to this intermediate level
                    lvlboxes[i].grow(Max(1, m_bufferSize/m_level_blockfactors[lvl]));
                    lvlboxes[i] &= Domains[lvl];

                    // (dfm 3/26/2001) since you're working on a set of
                    // faux levels which are a factor of m_blockFactor
                    // coarser than the levels you are _GENERATING_
                    // (in this case lvl+1), when you coarsen these
                    // boxes down to the next coarser level, you
                    // want to coarsen them by the refinement
                    // ratio between levels (lvl+1) and (lvl),
                    // instead of the ratio between (lvl) and
                    // (lvl-1).
                    lvlboxes[i].coarsen( m_nRefVect[lvl] ) ;
                  }
                     ...
                     const Vector<int>&   a_bufferSize) const
{
  ...
  for( int lvl = a_baseLevel ; lvl <= a_topLevel ; lvl++)
	...
	pnd.nestingRegion(a_bufferSize[lvl], a_domains[lvl], m_granularity);
	  ...
	  a_totalBufferSize[lvl+1] = a_totalBufferSize[lvl]*m_nRefVect[lvl+1];

          // note that we use the refinement ratio btwn
          // lvl+1 and lvl+2 in this case.  this is because
          // when this has been called from MeshRefine,
          // each level has already been brought the the
          // blocking-factor-enforcing coarseness of
          // lvl+1 coarsened by the blocking factor
          // (since that's where the grids produced by
          // the tags on lvl will eventually wind up)
          a_pnds[lvl+1].refine(m_nRefVect[lvl+1]);
        }
				...
				int   a_totalBufferSize) const
{
  if(m_PNDMode == 0)
  ...
  else
    {
 	...
 	if(growPeriodic != growBox){ //has periodic images
...
//  	    }
	  ...
	  for(images.begin(growPeriodic); images.ok(); ++images){
	    if(!images.box().isEmpty()){
	      if(!a_pnd.contains(images.box())) return false;
	    }
	  }
 	}
    }	
