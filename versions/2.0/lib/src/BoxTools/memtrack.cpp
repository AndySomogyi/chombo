...
long int            BYTES_PER_MEG      = 1024*1024;
...
void ReportUnfreedMemory(ostream& a_os)
{
  ...
  {
    // a_os<<"---------------------------------\n";
    // a_os<<"BaseFab-based memory allocation\n";
  ...
  peak += BitSet::peak + IntVectSet::peakcount*sizeof(IntVectSet);
      ...
      for (v = vectorList_->begin(); v != vectorList_->end(); v++)
        {
          ...
          if (*bytes != 0 && *bytes != 8)
            {
              string entry = "Vector";
              sprintf(temp, "%11s %-40s %12d b  %11.4f Mb  peak=%11.4f\n", entry.data(), v->first.data(), 
