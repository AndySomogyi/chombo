...
// an ugly macro named TIVS_NODESIZE to attain compilation with xlC.
// Currently, this only affects this file and the .H
// Certainly I'd love to fix it better than this...  (ndk)
...
void TreeIntVectSet::clearTree(TreeNode& tree)
{
  ...
  if (index_local.size() == 0) {tree.nodes = 0; return; }
  index_local[0] = 0;
...
Pool TreeIntVectSet::treeNodePool(sizeof(TreeNode[TIVS_NODESIZE]), "TreeIntVectSet Pool");
