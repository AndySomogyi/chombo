...
Real FArrayBox::dotProduct(const FArrayBox& a_fab2) const
{
 ...
 CH_assert(fab1.nComp() == a_fab2.nComp());
  ...
  if (!commonBox.isEmpty())
  {
    // FORT_DOTPRODUCT(CHF_REAL(dot),
    //                 CHF_CONST_FRA(fab1),
    //                 CHF_CONST_FRA(a_fab2),
    //                 CHF_BOX(commonBox),
    //                 CHF_CONST_INT(startcomp),
    //                 CHF_CONST_INT(endcomp));

    ForAllThisCBNNXC(Real, commonBox, startcomp, numcomp, a_fab2, startcomp)
    {
      dot += thisR * a_fab2R;
    } EndForTX
  }
                      ...
                      int           a_comp) const
{
  ...
  ForAllThisCBNN(Real,m_domain,a_comp,1)
  {
    ...
    int nr2 = nr;
    nr = nr2;
    // this silliness gets rid of unused variable warnings
    D_TERM(int ir = iR;,
           int jr = jR;,
           int kr = kR;);
                      ...
                      int           a_comp) const
{
  ...
  ForAllThisCBNN(Real,m_domain,a_comp,1)
  {
    ...
    int nr2 = nr;
    nr = nr2;
    // this little bit of silliness gets rid of unused variable warnings
    D_TERM(int ir = iR;,
           int jr = jR;,
           int kr = kR;);
