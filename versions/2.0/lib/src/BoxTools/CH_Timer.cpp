...
OldTimer::~OldTimer() {
  ...
  //  pout() << " OldTimer::~OldTimer() " << Name() << endl;
}
...
void OldTimer::TimerSummary_(const int itita) {
  ...
  pout() << "Total OldTimer Start/Stop Calls on rank " << rank << " = " << (int)NtotalStartStops << endl;
  pout() << "Estimated Single OldTimer Start/Stop-combo Cost = " << TimerCost << " sec." << endl;
  pout() << "Avg OldTimer Start/Stop Calls over all procs= " << avgStartStops << endl;
  ...
  pout() << " rank " << rank << " writing timmy.txt " << endl;
...
#ifdef TIMER_DEBUG    
  pout() << " size of ParentList = " << ParentList.size() << endl;
#endif
...
#ifdef TIMER_DEBUG    
    pout() << " ParentList size = " << ParentList.size() << endl;
#endif
  }
...
void OldTimer::writeDiagnosticTables(FILE *out, const double TimerCost) {
  ...
  //pout() << " Number of Diag Tables = " << diagTableSet.size() << endl;
...
struct elem
{
  ...
  double      time;
  elem(const TraceTimer* p, double t):val(p), time(t){;}
...
void TraceTimer::currentize() const
{
  ...
  if(m_last_WCtime_stamp != 0){
    ...
    double current = TimerGetTimeStampWC();
    (double&)m_accumulated_WCtime += current - m_last_WCtime_stamp;
    (double&)m_last_WCtime_stamp = current;
#ifdef CH_USE_MEMORY_TRACKING
    if(s_traceMemory)
      {
...
void TraceTimer::subReport(FILE* out, const char* header, double totalTime)
{     
  ...
  for(it.begin(); it.ok(); ++it)
    {
      ...
      if(strncmp(header, name, length) == 0){
        if((*it).val->isPruned()){
          ...
          double t = (*it).val->time();
          int rank = (*it).val->rank();
          subTime += t;
          fprintf(out, "  %8.3f %8lld  %s [%d] \n", t, (*it).val->m_count, name, rank);
        }
      }
    }
  if(subTime > 0)
    fprintf(out, "  %8.3f   %4.1f%%    Total\n", subTime, subTime/totalTime*100.0);
}
      
void TraceTimer::reportFullTree(FILE* out, const TraceTimer& timer, 
                                double totalTime, int depth)
{
  if(timer.m_pruned) return;
  double time = timer.m_accumulated_WCtime;
  
  if(depth < 20){
    for(int i=0; i<depth; ++i) fprintf(out,"   ");
    double percent = time/totalTime * 100.0;
    fprintf(out, "[%d] %s %.4f %4.1f%% %lld \n", timer.m_rank, timer.m_name, time, percent, timer.m_count);
  }
  Vector<int> ordering;
...
void TraceTimer::reportOneTree(FILE* out, const TraceTimer& timer)
{
  ...
  fprintf(out,"[%d]%s %.2f %lld\n", timer.m_rank, timer.m_name, time, timer.m_count);
  ...
  for(int i=0; i<children.size(); ++i)
    {
      ...
      if(!child.m_pruned)
        {
          double childtime = child.m_accumulated_WCtime;
          if(childtime > 0){
            subTime += childtime;
            double percent = childtime / time * 100.0;
            fprintf(out,"    %4.1f%% %7.2f %8lld %s [%d]\n", 
                    percent, childtime, child.m_count, child.m_name, child.m_rank);
          }
        } else {
         fprintf(out,"           pruned           \n");
         i=children.size();
  ...
  if(time > 0 && children.size() > 0){
    double totalPercent = subTime / time * 100.0;
...
void TraceTimer::start(char* mutex)
{
  ...
  *mutex = 1;
  ...
  m_last_WCtime_stamp = TimerGetTimeStampWC();
#ifdef  CH_USE_MEMORY_TRACKING
  if(s_traceMemory)
    {
      long long int peak;
...
double TraceTimer::stop(char* mutex)
{
  if(m_pruned) return 0;
  if(s_currentTimer[0] != this)
    MayDay::Error("TraceTimer::stop called while not parent");
  double diff = TimerGetTimeStampWC() - m_last_WCtime_stamp;
...
void TraceTimer::leafStop()
{ 
  if(m_pruned) return;
  m_accumulated_WCtime +=  TimerGetTimeStampWC() - m_last_WCtime_stamp;
  m_last_WCtime_stamp=0;
}
  ...
  for(int i=0; i<children.size(); ++i)
    {
      ...
      if(!child->isPruned())
        {
          double childtime = child->time();
          if(childtime/time < threshold) child->prune();
