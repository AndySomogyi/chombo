...
//  ch_offset_t offset[1];
  ...
  H5Gunlink(a_handle.groupID(), name.c_str()); //removes a pre-existing dataset.
  H5Eset_auto(efunc, edata);

  hid_t boxdataset   = H5Dcreate(a_handle.groupID(), name.c_str(),  a_handle.box_id,
...
{
#if ( defined(__IBMCPP__) && defined(CH_MPI) )
  // bugfix for broken HDF5 on IBM AIX
  H5dont_atexit() ;
#endif
...
{
  int err = open(a_filename, a_mode);
  if (err < 0 ){
    char buf[70];
