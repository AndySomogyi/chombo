...
///
int LoadBalance(Vector<int>& procs, const Vector<Box>& a_boxes,
                int a_numProc)
{
  Vector<long> computeLoads(a_boxes.size());
  ...
  int status;
  status = LoadBalance(procs, computeLoads, a_boxes, a_numProc);
...
// is enabled
///
int LoadBalance(Vector<int>&        procs,
                ...
                const int           a_numProc)
{
  ...
  int status = 0;
  procs.resize(0);
  procs.resize(a_computeLoads.size(), 0);

  int Nprocs = procs.size();

  if(a_numProc == 1 || Nprocs == 1) {
    return 0;
  }

  // first, compute 'total load'
  double totalLoad = 0; long long tload = 0;
  for(int index = 0; index < Nprocs; ++index)
    {
      totalLoad += a_computeLoads[index];
      tload += a_computeLoads[index];
      procs[index] = -index;
    }

  // figure out roughly single processor load goal
  double goal = totalLoad/a_numProc;

  // debugging prints
  //pout()<<"--- LoadBalance  --- Number of Boxes: "<<Nprocs << std::endl;
  //pout() << " Box sizes: " << std::endl;
  //for(int i=0; i < Nprocs; i++) pout() <<" "<<a_computeLoads[i];
  //pout() << std::endl;
  ...
  Vector<long long> loads(a_numProc, 0);
  int bin = 0;
  double dynamicLoad = totalLoad;
  double dynamicgoal = goal;
  double load = 0;
  long long numP;
  //pout() << " first time thru: dynamicLoad =" << dynamicLoad
  //       << " dynamicgoal = " << dynamicgoal << std::endl;

  for(int i=0; i < Nprocs; i++)
    {
      numP = a_computeLoads[i];
      if(bin != a_numProc-1)  // NOT last rank
        {
          load += numP;
          if(load >= dynamicgoal)
            {
              if( ( load - dynamicgoal ) > numP/2 )
                {
                  procs[i] = bin+1;
                  loads[bin+1] += numP;
                  load = numP;
              ...
              dynamicLoad -= (double)loads[bin];
              dynamicgoal = dynamicLoad/(double)(a_numProc-(bin+1));
              bin++;
            }
          else
            {  // last rank
  ...
  //pout()<<"tload = "<<(double)tload<< std::endl;
  //pout() << " loads: ";
  //for(int i=0; i<numProc(); i++) pout()<<" "<<(long)loads[i];
  //pout()<<std::endl;

  //Phase 2 of loadbalance, actual knapsack swapping to improve efficiency
  // first , scan for highest load processor
  //long long mload = loads[0];
  //for(int i=1; i<loads.size(); ++i) mload = Max(mload, loads[i]);

  //parallel inefficiency: sum of idle time of processors waiting for max load processor
  // dividedd by total load
  //double eff = 1 - ((double)(mload*a_numProc() - totalLoad))/totalLoad;
  //pout() << "loadbalance efficiency (before knapsack )= "<<eff
  //     << "  Nboxes=" << Nprocs
  //     <<std::endl;
  ...
  Vector<std::list<int> > neighbours(Nprocs);
      ...
      for(int ii=0; ii<Nprocs; ii++)
        {
          if(ii == i); //don't bother with self intersect
          else
            {
              ...
              if(sorted && bb.smallEnd(0) > b.bigEnd(0)) ii=Nprocs;
            }
        }
    }
  ...
  while (iter < 20 && swapcount > 0)
    {
      ...
      //pout()<<"Boxes: "<<Nprocs<<" same: "<<sameBoxes
