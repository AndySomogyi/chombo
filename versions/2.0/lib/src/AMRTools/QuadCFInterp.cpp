...
#ifndef NDEBUG
      Box domainCoar =  coarsen(m_domainFine.domainBox(), a_refRatio);
      IntVectSet coarIVSTotal;
      for(LayoutIterator lit = coarBoxes.layoutIterator(); lit.ok(); ++lit)
        {
          coarIVSTotal |= coarBoxes[lit()];
        }
#endif
      ...
      for(lit.begin(); lit.ok(); ++lit)
        {
          ...
          Box& coarBox = m_coarBoxes.ref(lit());
          coarBox.coarsen(m_refRatio);
          coarBox.grow(2);
      ...
      for(int ivar = a_variables.begin(); ivar <= a_variables.end(); ivar++)
        {
          ...
          if(a_qcfs.finePacked() && CH_SPACEDIM==3)
          ...
          else 
            {
              ...
              int jf, jc;
              Real xf, xc, x1, pc, update1=0, update2=0, update3=0;
