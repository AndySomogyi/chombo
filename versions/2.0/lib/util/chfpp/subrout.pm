...
### into one buffer
    ...
    while (defined $linebuf)
    {
        ...
        if ($linebuf =~ m/^\s*(subroutine|call)\s*/i)
        {
            ...
            $insubst = 1;
            $infirstline = 1;
            if( $1 =~ m/call/i ) { $incall  = 1; }
        }
        elsif ($linebuf =~ m/\)\s*call\s*/i)
        {
...
###and eight shall be the number of indents
    ...
    my $moduledecs = "";
    my $declarations = "";
    my ( $smdim,$boxdec,$boxarg,$compnm,$fabint,$uboundarg );
...
###create argument list and declaration stuff
    for(my $iarg = 0; $iarg <= $#subargs; $iarg++)
    {
        ...
        else
        {
            ...
            # except for the type
            elsif($singarg =~ /CHF\_(CONST\_)?V?([IRC])(1D)?/ig && $singarg !~ /\_V[IRC]1D/ig)
            {
                #NOTE: ChF code can use CHF_UBOUND[] on this, but not CHF_LBOUND[]
                $uboundarg     = "i".$singname."hi0";
                $arguments    .= $continstr.$comma.$singname;
                $arguments    .= $continstr.",".$uboundarg;
                $declarations .= $indentstr."integer $uboundarg";
                $declarations .= $indentstr.$singtype{uc($2)}." $singname\(";
                $declarations .= $continstr."0:$uboundarg\)";
            }
