...
#include "ChannelShockIBCF_F.H"

// Null constructor
ChannelShockIBC::ChannelShockIBC()
{
    m_isFortranCommonSet = false;
}

// Constructor which defines parameters used by Fortran routines
ChannelShockIBC::ChannelShockIBC(Real&       a_smallPressure,
                                 const Real& a_gamma,
                                 const Real& a_ms,
                                 const Real& a_position,
...
//   a_position      - Initial position of the shock
//   a_artvisc       - Artificial viscosity coefficient
void ChannelShockIBC::setFortranCommon(Real&       a_smallPressure,
                                       const Real& a_gamma,
                                       const Real& a_ms,
                                       const Real& a_position,
                                       const Real& a_artvisc)
{
  FORT_CHANNELSHOCKSETF(CHF_REAL(a_smallPressure),
                        CHF_CONST_REAL(a_gamma),
                        CHF_CONST_REAL(a_ms),
                        CHF_CONST_REAL(a_position),
...
void ChannelShockIBC::setFortranCommonSet()
{
  m_isFortranCommonSet = true;
}
...
PhysIBC* ChannelShockIBC::new_physIBC()
{
  ChannelShockIBC* retval = new ChannelShockIBC();
...
void ChannelShockIBC::initialize(LevelData<FArrayBox>& a_U)
{
       ...
       dit.ok(); ++dit)
  {
    ...
    FORT_CHANNELSHOCKINITF(CHF_FRA(U),
                           CHF_CONST_REAL(m_dx),
                           CHF_BOX(uBox));
  }
}
...
// Set boundary fluxes
void ChannelShockIBC::primBC(FArrayBox&            a_WGdnv,
                             ...
                             const Real&           a_time)
{
  ...
  if (!m_domain.isPeriodic(a_dir))
  {
    ...
    if (!m_domain.contains(tmp))
    {
      ...
      FORT_CHANNELSHOCKBCF(CHF_FRA(a_WGdnv),
...
void ChannelShockIBC::setBdrySlopes(FArrayBox&       a_dW,
                                    const FArrayBox& a_W,
                                    const int&       a_dir,
                                    const Real&      a_time)
{
  ...
  if (!m_domain.isPeriodic(a_dir))
  {
    ...
    if ((hasLo != 0) || (hasHi != 0))
    {
      FORT_CHANNELSHOCKSLOPEBCSF(CHF_FRA(a_dW),
                                 CHF_CONST_FRA(a_W),
                                 CHF_CONST_INT(a_dir),
                                 CHF_BOX(loBox),
                                 CHF_CONST_INT(hasLo),
...
void ChannelShockIBC::artViscBC(FArrayBox&       a_F,
                                const FArrayBox& a_U,
                                ...
                                const Real&      a_time)
{
  ...
  if (hasLo == 1)
  {
    ...
    FORT_CHANNELSHOCKARTVISCF(CHF_FRA(a_F),
                              CHF_CONST_FRA(a_U),
