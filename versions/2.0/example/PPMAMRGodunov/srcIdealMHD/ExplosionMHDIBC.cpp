...
//   a_pressure_jump - p(explosion)/p0
//   a_density_jump  - rho(explosion)/p0
//   p0              - ambient pressure 
//   rho0            - ambient density
//   B_direction     - direction of magnetic field
                           ...
                           const Real&     a_artvisc)
{
  ...
  FORT_EXPLOSIONSETF(CHF_REAL(a_smallPressure),
                     ...
                     CHF_CONST_REAL(a_pressure_jump),
                     CHF_CONST_REAL(a_density_jump),
                     CHF_CONST_REAL(a_p0),
                     CHF_CONST_REAL(a_rho0),
                     CHF_CONST_INT(a_B_direction),
                     CHF_CONST_REAL(a_B_magnitude),
                          ...
                          const Real&           a_time)
{
  ...
  if (!m_domain.isPeriodic(a_dir))
  {
    ...
    if (!m_domain.contains(tmp))
    {
      ...
      FORT_EXPLOSIONMHDPRIMBCF(CHF_FRA(a_WGdnv),
                                 ...
                                 const Real&      a_time)
{
  ...
  if (!m_domain.isPeriodic(a_dir))
  {
    ...
    if ((hasLo != 0) || (hasHi != 0))
    {
      FORT_EXPLOSIONMHDSLOPEBCSF(CHF_FRA(a_dW),
                                 CHF_CONST_FRA(a_W),
                                 CHF_CONST_REAL(m_dx),
                                 CHF_CONST_INT(a_dir),
                                 CHF_BOX(loBox),
