...
void amrGodunov()
{
  ...
  // default is to project B
  bool projectBField = true;
  ppgodunov.query("project_BField", projectBField);
  ...
  if (ppgodunov.contains("problem"))
  {
    ...
    {
      // Explosion problem
      Real density_jump, pressure_jump, p0, rho0;
      ppgodunov.get("density_jump",density_jump);
      ppgodunov.get("pressure_jump", pressure_jump);
      ppgodunov.get("p0", p0);
      ppgodunov.get("rho0", rho0);
      int B_direction;
      Real B_magnitude;
      ppgodunov.get("B_direction", B_direction);
      ppgodunov.get("B_magnitude", B_magnitude);
      ...
      if (verbosity >= 2)
      {
        pout() << "pressure jump = " << pressure_jump << endl;
        pout() << "ambient pressure = " << p0 << endl;
        pout() << "density jump = " << density_jump << endl;
        pout() << "ambient density = " << rho0 << endl;
        pout() << "B-field direction = " << B_direction << endl;
        pout() << "B-field magnitude = " << B_magnitude << endl;
