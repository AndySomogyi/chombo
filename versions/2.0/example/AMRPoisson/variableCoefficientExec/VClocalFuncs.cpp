...
void getPoissonParameters(VCPoissonParameters&  a_params)
{
  ...
  pp.get("alpha",a_params.alphaFactor);
  pp.get("beta", a_params.betaFactor);
  ...
  for (DataIterator dit = a_rhs.dataIterator(); dit.ok(); ++dit)
    {
      ...
      FORT_GETLOFPHI(CHF_FRA1(thisRHS,comp),
                     ...
                     CHF_CONST_REAL(a_params.alphaFactor),
                     CHF_CONST_REAL(a_params.betaFactor),
                     CHF_BOX(thisBox));
                      ...
                      const VCPoissonParameters&                     a_params)
{
  ...
  VCAMRPoissonOpFactory* opFactory = new VCAMRPoissonOpFactory;
  ...
  opFactory->setAlpha(a_alpha);
  opFactory->setBeta(a_beta);
