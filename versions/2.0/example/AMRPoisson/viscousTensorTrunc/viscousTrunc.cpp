              ...
              const PoissonParameters&             a_params)
{
  ...
  Vector<ViscousTensorOp*>         ebamrpo(nlevels, NULL);
  Vector<RefCountedPtr<LevelData<FluxBox> > >      etaExac(nlevels);
  Vector<RefCountedPtr<LevelData<FluxBox> > >   lambdaExac(nlevels);
  Vector<RefCountedPtr<LevelData<FArrayBox> > >   betaExac(nlevels);
  ...
  for(int ilev = 0; ilev < nlevels; ilev++)
    {
      ...
      velExac[ilev] = new LevelData<FArrayBox>(a_grids[ilev], SpaceDim,3*IntVect::Unit);
      etaExac[ilev]    = RefCountedPtr<LevelData<FluxBox>   >(new LevelData<FluxBox>  (a_grids[ilev], 1, IntVect::Zero));
      lambdaExac[ilev] = RefCountedPtr<LevelData<FluxBox>   >(new LevelData<FluxBox>  (a_grids[ilev], 1, IntVect::Zero));
      betaExac[ilev]   = RefCountedPtr<LevelData<FArrayBox> >(new LevelData<FArrayBox>(a_grids[ilev], 1, IntVect::Zero));
      //set vel = velExact, rhs=lvelexact  This makes AMRResidual return lvelexact-Lvel
      setViscousCoefs(*etaExac[ilev], *lambdaExac[ilev], *betaExac[ilev], dxLev, a_params);
      setVelViscous(  *velExac[ilev], dxLev, a_params);
      setKLVViscous(  *klbExac[ilev], dxLev, a_params);
      dxLev /=      a_params.refRatio[ilev];
    }
                                         
  ViscousTensorOpFactory opFactory(a_grids, etaExac, lambdaExac,
                                   a_params.alpha,  betaExac,
                                   a_params.refRatio,
  ...
  for(int ilev = 0; ilev < nlevels; ilev++)
    {
      ...
      if(nlevels == 1)
        {
          //single level calc
          ebamrpo[ilev]->residual(*a_error[ilev], *velExac[ilev], *klbExac[ilev], homogeneous);
        }
      else if(ilev == nlevels-1)  //at finest level  of multi level calc
        {
          ebamrpo[ilev]->AMRResidualNF(*a_error[ilev], *velExac[ilev], *velExac[ilev-1],  
                                       *klbExac[ilev], homogeneous);
        }
      else if(ilev == 0)  //at lowest level of multi level calc
        {
          ebamrpo[ilev]->AMRResidualNC(*a_error[ilev], *velExac[ilev+1], *velExac[ilev],
                                       *klbExac[ilev], homogeneous, ebamrpo[ilev+1]);
        }
      else  //mid level of multi level calc 
        {
          ebamrpo[ilev]->AMRResidual  (*a_error[ilev], *velExac[ilev+1], *velExac[ilev], *velExac[ilev-1],
                                       *klbExac[ilev],   homogeneous, ebamrpo[ilev+1]);
