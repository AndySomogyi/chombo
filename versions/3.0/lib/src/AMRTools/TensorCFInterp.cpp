...
TensorCFInterp::clear()
{
  ...
  for (int idir=0; idir<SpaceDim; idir++)
    {
      m_loCoarCopiers[idir].clear();
      m_hiCoarCopiers[idir].clear();
    }
}
  ...
  if(m_level > 0)
    {
      ...
      for(int idir = 0; idir < SpaceDim; idir++)
        {
          ...
          m_loCoarBoxes[idir].deepCopy(a_fineBoxes);
          m_hiCoarBoxes[idir].deepCopy(a_fineBoxes);
        }
      ...
      for(lit.begin(); lit.ok(); ++lit)
        {
          const Box& fineBox = a_fineBoxes[lit()];
          for(int idir = 0; idir < SpaceDim; idir++)
            {
              Box coarBox   = coarsen(fineBox, m_refRatio);
              Box coarBoxHi = adjCellHi(coarBox,idir,1);
              Box coarBoxLo = adjCellLo(coarBox,idir,1);
              //give em a ghost cell on non-idir sides
              coarBoxLo.grow(1);
              coarBoxLo.grow(idir, -1);
              coarBoxHi.grow(1);
              coarBoxHi.grow(idir, -1);

              m_loCoarBoxes[idir].ref(lit()) = coarBoxLo;
              m_hiCoarBoxes[idir].ref(lit()) = coarBoxHi;
            }
        }

      for(int idir = 0; idir < SpaceDim; idir++)
        {
          m_loCoarBoxes[idir].close();
          m_hiCoarBoxes[idir].close();
          m_loCoarBuffer[idir].define(m_loCoarBoxes[idir],m_nComp);
          m_loCoarCopiers[idir].define(coarBoxes, m_loCoarBoxes[idir]);
          m_hiCoarBuffer[idir].define(m_hiCoarBoxes[idir],m_nComp);
          m_hiCoarCopiers[idir].define(coarBoxes, m_hiCoarBoxes[idir]);
        }
  ...
  if(m_level > 0)
    {
      for(int idir = 0; idir < SpaceDim; idir++)
        {
          //put coarse data into buffers
          a_phic.copyTo(a_phic.interval(),
                        m_loCoarBuffer[idir],
                        m_loCoarBuffer[idir].interval(),
                        m_loCoarCopiers[idir]);
          a_phic.copyTo(a_phic.interval(),
                        m_hiCoarBuffer[idir],
                        m_hiCoarBuffer[idir].interval(),
                        m_hiCoarCopiers[idir]);
          ...
          for (int idir=0; idir<SpaceDim; idir++)
            {
              ...
              {
                const BaseFab<Real> & loPhiC = m_loCoarBuffer[idir][datIndGlo];
              ...
              //hi side cfinterp
              {
                const BaseFab<Real> & hiPhiC = m_hiCoarBuffer[idir][datIndGlo];
  ...
  if(m_level > 0)
    {
      for(int idir = 0; idir < SpaceDim; idir++)
        {
          // instead of putting coarse data into buffers, as in the
          // non-homogeneous case, just set coarse buffers all to 0.
          DataIterator loDit = m_loCoarBuffer[idir].dataIterator();
          for (loDit.begin(); loDit.ok(); ++loDit)
            {
              m_loCoarBuffer[idir][loDit()].setVal(0.0);
            }

          DataIterator hiDit = m_hiCoarBuffer[idir].dataIterator();
          for (hiDit.begin(); hiDit.ok(); ++hiDit)
            {
              m_hiCoarBuffer[idir][hiDit()].setVal(0.0);
            }
        }
