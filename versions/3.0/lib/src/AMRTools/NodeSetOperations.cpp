                      ...
                      const ProblemDomain& a_domain)
{
  ...
  for (LayoutIterator litDst = destNodeIndices.layoutIterator(); litDst.ok(); ++litDst)
    {
      Box& bx = destNodeIndices.ref(litDst());
      for (int idir = 0; idir < SpaceDim; idir++)
        bx.growHi(idir, 1);
    }
  ...
  for (DataIterator ditSrc = srcCount.dataIterator(); ditSrc.ok(); ++ditSrc)
    {
      ...
      FORT_HALVEINTFACES(CHF_FIA(srcFab),
                         CHF_BOX(srcFab.box()));
    }
                         ...
                         const BoxLayout&   a_gridsCells)
{
  ...
  for (LayoutIterator lit = a_gridsCells.layoutIterator(); lit.ok(); ++lit)
    {
      const Box& bxCells = a_gridsCells.get(lit());
      CH_assert(bxCells.ixType() == IndexType::TheCellType());
      Box bxNodes = surroundingNodes(bxCells);
      a_gridsNodes.ref(lit()) = bxNodes;
    }
  a_gridsNodes.close();
}
                      ...
                      const BoxLayout&   a_gridsNodes)
{
  ...
  for (LayoutIterator lit = a_gridsNodes.layoutIterator(); lit.ok(); ++lit)
    {
      const Box& bxNodes = a_gridsNodes.get(lit());
      CH_assert(bxNodes.ixType() == IndexType::TheNodeType());
      Box bxCells = enclosedCells(bxNodes);
      a_gridsCells.ref(lit()) = bxCells;
    }
  a_gridsCells.close();
}
