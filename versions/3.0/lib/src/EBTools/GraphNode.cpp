...
GraphNodeImplem&
GraphNodeImplem::
operator=(const GraphNodeImplem& a_impin)
{
  if(&a_impin != this)
    {
      for(int iarc = 0; iarc < 2*SpaceDim; iarc++)
...
/*******************************/
void
GraphNode::addIrregularNode(const GraphNodeImplem& a_nodein, int cellIndex)
{
...
void
GraphNode::addIrregularNode(const GraphNodeImplem& a_nodein)
{
  if(isRegular() || isCovered())
    {
      m_cellList = new Vector<GraphNodeImplem>();
...
Vector<FaceIndex> GraphNode::
getFaces(const IntVect&        a_this,
         const int&            a_idir,
...
operator=(const GraphNode& a_nodein)
{
  if(this != &a_nodein)
    {
      clear();
      ...
      if((a_nodein.isRegular()) || (a_nodein.isCovered()))
        {
...
/*******************************/
GraphNode::
GraphNode(const GraphNode& a_nodein)
{
  //if node is regular or covered, just copy the pointer
  //otherwise, append the list of nodes
  if((a_nodein.isRegular()) || (a_nodein.isCovered()))
...
/*******************************/
VolIndex
GraphNode::coarsen(const VolIndex& a_fineVoF) const
{
  VolIndex retval;
  ...
  if(isRegular() || isCovered())
  ...
  else
    {
      ...
      int cellIndex = nodes[inode].m_coarserNode;
      retval = VolIndex(ivCoar, cellIndex);
    }
  return retval;
}
/*******************************/
int
GraphNode::linearSize() const
{
  int retval;
  if(isRegular() || isCovered())
    {
...
/*******************************/
void
GraphNode::linearOut(void*  a_buf) const
{
  int secretCode;
  if(isRegular())
    {
      //secret code for regular.
...
GraphNode::linearIn(void* a_buf)
{
  ...
  if(secretCode == 1)
    ...
    {
      //secret code for irregular.  I had to pick something.
...
/*******************************/
int
GraphNodeImplem::linearSize() const
{
  int linSize = 0;
...
/*******************************/
void
GraphNodeImplem::linearOut(void*  a_buf) const
{
  int* intbuf = (int*) a_buf;
  int linSize = 0;
...
/*******************************/
void
GraphNodeImplem::linearIn(void* a_buf)
{
  int* intbuf = (int*) a_buf;
  int linSize = 0;
