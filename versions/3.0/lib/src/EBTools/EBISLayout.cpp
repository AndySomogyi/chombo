                         ...
                         const LevelData<EBData> & a_data)
{
  ...
  for(LayoutIterator lit = a_grids.layoutIterator(); lit.ok(); ++lit)
    {
      Box& thisBox = m_blGhostDom.ref(lit());
      thisBox.grow(a_nghost);
      thisBox &= m_domain;
    }
  m_blGhostDom.close();
  if(s_verbose)
  ...
  Interval interv(0, 0);
  LevelData<EBGraph> localGraph(a_grids, 1, ivghostgraph, graphFact);
  ...
  LevelData<EBData> localData(a_grids, 1, ivghostdata, dataFact);
  ...
  for(int ilev = 0; ilev < nlevels; ilev++)
    {
      ...
      ebisPtr->fillEBISLayout(m_coarLevels[ilev], coarBLDomain, coarDomain, m_nghost);
      irat *= 2;
    }
}
/****************/
