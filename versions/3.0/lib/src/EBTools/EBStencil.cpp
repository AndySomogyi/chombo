...
//  CH_START(t1);
  ...
  BaseIVFAB<Real> baseivfabPhi(ivsPhi, ebgraphPhi, a_varDest+1);
...
void EBStencil::apply(EBCellFAB& a_lofphi, const EBCellFAB& a_phi, bool a_incrementOnly) const
{
  ...
  for (int isrc = 0; isrc < m_ebstencil.size(); isrc++)
    {
      ebstencil_t ebstencil = m_ebstencil[isrc];
      Real* lphiPtr = NULL;
  ...
  for (int isrc = 0; isrc < m_ebstencil.size(); isrc++)
    {
      ...
      Real lphi = lambda*a_alpha*alphaWeight*phi;
      //single-valued
      for (int isingle = 0; isingle < ebstencil.single.size(); isingle++)
        {
          const int & offset = ebstencil.single[isingle].offset;
...
/**************/
void
EBStencil::cachePhi(const EBCellFAB& a_phi) const
{
//   CH_assert(m_cache.size() == m_ebstencil.size());

  CH_assert(a_phi.getSingleValuedFAB().box()    == m_phiBox);
  const Real* singleValuedPtrPhi =    a_phi.getSingleValuedFAB().dataPtr();
  const Real*  multiValuedPtrPhi =    a_phi.getMultiValuedFAB().dataPtr(0);
...
void
EBStencil::cache(const EBCellFAB& a_lph) const
{
//   CH_assert(m_cache.size() == m_ebstencil.size());

  CH_assert(a_lph.getSingleValuedFAB().box()    == m_lphBox);
  const Real* singleValuedPtrPhi =    a_lph.getSingleValuedFAB().dataPtr();
  const Real*  multiValuedPtrPhi =    a_lph.getMultiValuedFAB().dataPtr(0);

  for (int isrc = 0; isrc < m_ebstencil.size(); isrc++)
    {
      if(m_destTerms[isrc].multiValued)
        {
          m_cache[isrc] = *(multiValuedPtrPhi + m_destTerms[isrc].offset);
        }
      else
        {
          m_cache[isrc] = *(singleValuedPtrPhi + m_destTerms[isrc].offset);
        }
    }
}
/**************/
/**************/
void
EBStencil::uncachePhi(EBCellFAB& a_phi) const
{
  CH_assert(a_phi.getSingleValuedFAB().box()    == m_phiBox);
  Real* singleValuedPtrPhi =    a_phi.getSingleValuedFAB().dataPtr();
  Real*  multiValuedPtrPhi =    a_phi.getMultiValuedFAB().dataPtr(0);
...
/**************/
void
EBStencil::uncache(EBCellFAB& a_lph) const
{
  CH_assert(a_lph.getSingleValuedFAB().box()    == m_lphBox);
  Real* singleValuedPtrPhi =    a_lph.getSingleValuedFAB().dataPtr();
  Real*  multiValuedPtrPhi =    a_lph.getMultiValuedFAB().dataPtr(0);
  ...
  for (int isrc = 0; isrc < m_ebstencil.size(); isrc++)
    {
      if(m_destTerms[isrc].multiValued)
        {
          phiPtr  = multiValuedPtrPhi + m_destTerms[isrc].offset;
        }
      else
        {
          phiPtr  = singleValuedPtrPhi + m_destTerms[isrc].offset;
        }
