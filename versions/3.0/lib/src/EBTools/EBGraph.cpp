...
/*******************************/
IntVectSet EBGraphImplem::
getIrregCells(const Box& a_subbox) const
{
...
/*******************************/
IntVectSet EBGraphImplem::
getMultiCells(const Box& a_subbox) const
{
  static IntVectSet emptySet;
  ...
  for(int ivecIrreg = 0; ivecIrreg < a_irregGraph.size(); ivecIrreg++)
    {
      ...
      if(!foundNode)
        {
          MayDay::Error("internal error in construction");
        }
...
/*******************************/
const Box& EBGraphImplem::
getRegion() const
{
  return m_region;
}
/*******************************/
const ProblemDomain& EBGraphImplem::
getDomain() const
{
  return m_domain;
}
...
/*******************************/
Vector<FaceIndex>
EBGraphImplem::getAllFaces(const IntVect& a_iv,
                           const int& a_idir,
...
/*******************************/
int
EBGraphImplem::size(const Box& a_region, const Interval& a_comps) const
{
  CH_assert(isDefined());
  CH_assert(isDomainSet());
...
/*******************************/
void
EBGraphImplem::linearOut(void* a_buf,
                         const Box& a_region,
...
/*******************************/
void
EBGraphImplem::linearIn(void*           a_buf,
                        const Box&      a_region,
...
/*******************************/
Vector<FaceIndex> EBGraphImplem::
...
/*******************************/
Vector<FaceIndex> EBGraphImplem::
refine(const FaceIndex& a_coarFace, const EBGraphImplem& a_fineGraph) const
{
#ifndef NDEBUG
...
/*******************************/
bool
EBGraphImplem::isConnected(const VolIndex& a_vof1,
...
/*******************************/
Vector<Vector<VolIndex> >  EBGraphImplem::
...
/*******************************/
void EBGraphImplem::
coarsenFaces(const EBGraphImplem& a_coarGhostGraph,
             const EBGraphImplem& a_fineGraph)
{
...
/*******************************/
void EBGraphImplem::
fixFineToCoarse(EBGraphImplem& a_fineGraph) const
{
  if(hasIrregular())
    {
      for(BoxIterator bit(m_region); bit.ok(); ++bit)
          ...
          if(isIrregular(bit()))
            {
              ...
              const Vector<GraphNodeImplem>&
                nodesCoar = *(m_graph(ivCoar, 0).m_cellList);
              for(int icoar = 0; icoar < nodesCoar.size(); icoar++)
                {
...
int
EBGraphImplem::numVoFs(const IntVect& a_iv) const
{
//   Vector<VolIndex> vofs = getVoFs(a_iv);
//   return vofs.size();
  CH_assert(isDefined());
...
/*******************************/
int
EBGraphImplem::numVoFs(const Box& a_box) const
{
  CH_assert(isDefined());
  CH_assert(isDomainSet());
  int count = 0;
  if(m_tag == AllRegular)
    ...
    {
      IntVectSet m = *m_irregIVS & a_box;
      count = m.numPts();
      if(count == 0) return 0;
      m = *m_multiIVS & a_box;
      for(IVSIterator it(m); it.ok(); ++it)
        {
          count += numVoFs(it());
        }
...
/*******************************/
void
EBGraph::define(const Box& a_box)
{
  m_implem->define(a_box);
}
/*******************************/
int
EBGraph::size(const Box& R, const Interval& comps) const
{
  return  (m_implem->size(R, comps));
}
/*******************************/
void
EBGraph::linearOut(void* buf, const Box& R, const Interval& comps) const
{
  m_implem->linearOut(buf, R, comps);
}
/*******************************/
void
EBGraph::linearIn(void* buf, const Box& R, const Interval& comps)
{
...
{
  a_sizes[0] = item.size(box, comps)/sizeof(int);
}
                                                     ...
                                                     const EBGraph& parent) const
{
  if(a_loc == 0 || a_loc == 1){
    ...
    std::map<Box, FaceIterator>& fmap = m_boxFaceCache[a_loc*CH_SPACEDIM+a_direction];
    FaceIterator& it = fmap[a_region];
    if(!it.isDefined()){
      IntVectSet ivs ;
      ivs=getIrregCells(a_region);
      it.define(ivs, parent, a_direction, a_loc);
    }
    return it;
  }
  return FaceIteratorCache(getIrregCells(a_region), a_direction, a_loc, parent);
}
const FaceIterator& EBGraphImplem::FaceIteratorCache(const IntVectSet& a_ivs,
                                                     int a_direction,
                                                     FaceStop::WhichFaces a_loc,
                                                     const EBGraph& parent) const
{
  std::map<IntVectSet, FaceIterator>& fmap = m_faceCache[a_loc*CH_SPACEDIM+a_direction];
  FaceIterator& it = fmap[a_ivs];
  if(!it.isDefined()){
    it.define(a_ivs, parent, a_direction, a_loc);
  }
  return it;
}

const VoFIterator& EBGraphImplem::VoFIteratorCache(const IntVectSet& a_ivs,
                                                   const EBGraph& parent) const
{
  VoFIterator& it = m_vofIteratorCache[a_ivs];
  if(!it.isDefined()){
    it.define(a_ivs, parent);
  }
  return it;
}
