                ...
                const VolIndex& a_vof)
{
  ...
  for(int ivof = 0; ivof  < a_vofStencil.size(); ivof++)
    {
      if(a_vofStencil.vof(ivof) == a_vof)
        {
          found = true;
...
Real
EBArith::extrapToDomainFace(const FaceIndex&      a_bndryFace,
...
//     {
//       extrapValue = sign(a_side)*(4.*nearVal - 3.*farVal + farFarVal)/2.;
...
Real EBArith::getFaceVelForDivFreeCell(const FaceIndex&      a_face,
                                       const VolIndex&       a_vof,
                                       const int&            a_idir,
                                       const Side::LoHiSide& a_side,
                                       ...
                                       const EBISBox&        a_ebisBox)
{
  //set outflow condition to set divergence to zero
  //extrapolate at outflow
  //add up contributions to divergence of all faces
  //not the outflow face and set  the outflow face velocity
  //to set the outflow divergence to zero.
  Real sumNotOutFlow = 0;
  for(int faceDir = 0; faceDir < SpaceDim; faceDir++)
    {
      for(SideIterator sit; sit.ok(); ++sit)
        {
          if((a_side != sit()) || (faceDir != a_idir))
            {
              Vector<FaceIndex> faces = a_ebisBox.getFaces(a_vof, faceDir, sit());
              for(int iface = 0; iface < faces.size(); iface++)
                {
                  //need to interpolate the velocity to the centroid of the  face
                  //because that  is what the divergence operator does.
                  Real velFace = EBArith::interpolateVel(a_vel[faceDir], a_ebisBox, faces[iface]);
                  Real alpha = a_ebisBox.areaFrac(faces[iface]);
                  int isign = sign(sit());
                  sumNotOutFlow += Real(isign)*alpha*velFace/a_dx[faceDir];
                }
            }
        }
    }
  Real faceFlux;
  Real areaFrac = a_ebisBox.areaFrac(a_face);
  if(areaFrac > 0)
    {
      //this is the velocity at the centroid of the face.  need to
      //devolve it into the vel at the cell center because  that is what is expected
      Real centroidVel = -sumNotOutFlow*a_dx[a_idir]/(areaFrac);
      Real cellVel = EBArith::deInterpolateVel(centroidVel, a_vel[a_idir], a_ebisBox, a_face);
      faceFlux = cellVel;
    }
  else
    {
                        ...
                        int a_ivar)
{
  ...
  if (a_stencil.size() != 0)
    ...
    {
      // factor we use for stencil comparison
      Real volSum = 0.0;
      // next stencil candidate
      VoFStencil curStencil;
      Real curWeight, curVolSum;
      IntVect curQuadrant;
      ...
      for (int idir=0; idir<SpaceDim; idir++)
        {
          ...
          if (curQuadrant != quadrant)
            {
              curStencil.clear();
              EBArith::getLeastSquaresGradSten(curStencil, curWeight, a_normal,
                                               a_centroid, curQuadrant, a_vof,
                                               a_ebisBox, a_dx, a_domain,
                                               a_ivar);

              if(curStencil.size() != 0)
                {
                  curVolSum = 0.0;
                  for (int isten=0; isten<curStencil.size(); isten++)
                    {
                      curVolSum += a_ebisBox.volFrac(curStencil.vof(isten));
                    }
                  if (curVolSum > volSum)
                    {
  ...
  if (!dropOrder)
    {
      ...
      Vector<RealVect> aTransA(SpaceDim), invATransA(SpaceDim);
      for (int idir = 0; idir < SpaceDim; idir++)
        {
      ...
      Vector<RealVect> invATransAdeltaX(stenSize);
      for (int idir = 0; idir < SpaceDim; idir++)
        {
          for (int isten = 0; isten < stenSize; isten++)
            {
              for (int jdir = 0; jdir < SpaceDim; jdir++)
                {
                  invATransAdeltaX[isten][idir] = invATransAdeltaX[isten][idir]
                    + invATransA[idir][jdir] * deltaX[isten][jdir];
                }
            }
        }
...
void
EBArith::
getLeastSquaresGradStenAll(VoFStencil&          a_stencil,
                           Real&                a_weight,
                           const RealVect&      a_normal,
                           const RealVect&      a_centroid,
                           const VolIndex&      a_vof,
                           ...
                           int                  a_ivar)
{
  ...
  for (int inbr=0; inbr<numNeighbors; inbr++)
    {
      VolIndex ivof;
      if (a_ebisBox.getDomain().contains(ivSten[inbr]) &&
          a_ebisBox.getVoFs(ivSten[inbr]).size() > 0)
        {
          ivof = a_ebisBox.getVoFs(ivSten[inbr])[0];
        }

      bool monotonePath = EBArith::monotonePathVoFToCellVoF(ivof,
                                                            a_vof,
                                                            ivSten[inbr],
                                                            a_ebisBox);
      if (monotonePath)
        {
          volSten.push_back(ivof);
        }
    }
