...
void
DisjointBoxLayout::closeN(RefCountedPtr<Vector<Vector<std::pair<int, unsigned int> > > > neighbors)
...
void DisjointBoxLayout::computeNeighbors()
{
  ...
  std::list<std::pair<int, unsigned int> > periodicImages;
              ...
              for (shiftIt.begin(); shiftIt.ok(); ++shiftIt)
                {
                  ...
                  periodicImages.push_back(std::pair<int, unsigned int>(shiftIt.index(), index));
                  shiftedBox.shift(-shiftVect);
  ...
  m_neighbors = RefCountedPtr<Vector<Vector<std::pair<int, unsigned int> > > >(
            new Vector<Vector<std::pair<int, unsigned int> > >());
  DataIterator dit = dataIterator();
  m_neighbors->resize(dit.size());
  for(DataIterator dit=dataIterator(); dit.ok(); ++dit)
    {
      Box gbox = get(dit());
      gbox.grow(1);
      Vector<std::pair<int, unsigned int> >& neighbors = (*m_neighbors)[lindex(dit())];
      int low =  gbox.smallEnd()[0] - maxI;
      int high = gbox.smallEnd()[0] + maxI + 1;
      for(id=start; id<end; ++id)
        {
          if(id != index(dit())){ //don't include yourself as neighbor
            ...
            if(gbox.intersectsNotEmpty(b))
              {
                neighbors.push_back(std::pair<int, unsigned int>(-1, boxes[id].index));
              }
          }
        }
      //now run through periodic boxes.
      std::list<std::pair<int, unsigned int> >::iterator it;
      for(it=periodicImages.begin(); it!=periodicImages.end(); ++it)
        {
          Box b = (*m_boxes)[(*it).second].box;
          m_physDomain.shiftIt(b, (*it).first);
          if(gbox.intersectsNotEmpty(b))
            {
              pair<int, unsigned int> entry(*it);
              entry.second=boxes[it->second].index;
