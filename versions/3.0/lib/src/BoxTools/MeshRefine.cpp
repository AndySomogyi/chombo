      ...
      if (variableNRef &&  (m_blockFactor < maxNRef))
        {
          pout () << "MeshRefine error: If nRef varies, "
                  << "blockFactor may not be less than max ref ratio." << endl;
      ...
      if( TopLevel+1 > a_baseLevel )
        {
          ...
          // GHM 7/30/01 the buffer size argument to makePND needs to be coarsened by
          // the BlockFactor, since all the other arguments have been so coarsened.
          // If not, a circumstance may occur where a set of grids defined with a certain
          // set of tags does not "refine" to itself given the exact same set of tags.
