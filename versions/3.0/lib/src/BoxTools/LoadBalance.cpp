                ...
                const int                a_LBnumProc)
{
      ...
      if (NboxesLeftEqualsNprocsLeft)
        {
          ...
          if (bin > a_LBnumProc) {  // Not sure this could even happen...
  ...
  if(procID() == 0) {
    ...
    for(int i=1; i<loads.size(); ++i) {
      if (loads[i] > maxload) { maxload=loads[i]; rankmax=i; }
      if (loads[i] < minload) { minload=loads[i]; rankmin=i; }
    }
    ...
    if (Nboxes > NminNumberBoxesToDiagPrint) {
      ...
      for(int i=0; i<a_LBnumProc; i++) {
        ...
        int NboxCount=0;
        for(int jbox=0; jbox < Nboxes; jbox++) {  // count up number of boxes on proc i
          if (a_procAssignments[jbox] == i) NboxCount++;
        }
