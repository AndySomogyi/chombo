...
NeighborIterator::NeighborIterator(const DisjointBoxLayout& dbl)
  :m_dblPtr(&dbl), m_lindex(-1, dbl.m_layout)
{;}

void NeighborIterator::begin(const DataIndex& a_dataIndex)
{
  const Vector<Vector<std::pair<int, unsigned int> > >&  vv = *(m_dblPtr->m_neighbors);
  const std::vector<std::pair<int, unsigned int> >& v = vv[m_dblPtr->lindex(a_dataIndex)].constStdVector();
  m_current = v.begin();
  m_end = v.end();
  if(ok())
    {
       m_lindex.m_index = m_current->second;
    }
}
