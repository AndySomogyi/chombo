...
Box::numPtsOK (long& N) const
{
...
#if CH_SPACEDIM == 1
    N = bigend[0] - smallend[0] + 1;
    return true;
#elif CH_SPACEDIM == 2
    N = (bigend[0]-smallend[0]+1)*(bigend[1] - smallend[1] +1);
    return true;
#elif CH_SPACEDIM == 3
    N = (bigend[0]-smallend[0]+1)*(bigend[1] - smallend[1] +1);
    int N1 =   (bigend[2]-smallend[2]+1);
    if(N1 <= LONG_MAX/N) N*=N1;
    else return false;
    return true;
#elif CH_SPACEDIM == 4
    N = (bigend[0]-smallend[0]+1)*(bigend[1] - smallend[1] +1);
    long N1 = (bigend[2]-smallend[2]+1)*(bigend[3]-smallend[3]+1);
    if (N1 <= LONG_MAX/N)
      {
        N *= N1;
      }
    ...
    long N1 = (bigend[3]-smallend[3]+1)*(bigend[4] - smallend[4] +1);
    if (N1 <= LONG_MAX/N)
      {
        N *= N1;
      }
