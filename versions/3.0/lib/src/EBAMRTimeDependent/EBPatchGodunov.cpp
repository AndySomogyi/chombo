...
IntVect EBPatchGodunov::s_debugIV          = IntVect(D_DECL(7, 0, 0));
  ...
  for(int ivar = 0; ivar < a_numPrim; ivar++)
    {
      ...
      if (hasVoFLitNorm && hasVoFBigNorm)
        ...
        {//change methods, still second order
          // pout() << "EBPatchGodunov::doing 1D extrap to covered for: vof = " << a_vof
          //        << "; side = " << a_sd << "; faceDir = " << a_faceDir << endl;
          Real deltaW;
          coveredExtrapSlopes(deltaW,vof[1][0],a_primState,a_faceDir,ivar);
        ...
        {//drop order
          //pout() << "EBPatchGodunov::pointExtrapToCovered2D:lit: vof " << a_vof << " drops order " << endl;
          a_extrapVal[ivar] = wLitNorm;
        }
      else if (hasVoFBigNorm)
        {
          //pout() << "EBPatchGodunov::pointExtrapToCovered2D:big: vof " << a_vof << " drops order " << endl;
          a_extrapVal[ivar] = wBigNorm;
        }
      else
        {
          //pout() << "EBPatchGodunov::pointExtrapToCovered2D:none: vof " << a_vof << " drops order " << endl;
  ...
  for(int ivar = 0; ivar < a_numPrim; ivar++)
    {
      if(hasAllVoFs)
        ...
        {
          //pout() << "EBPatchGodunov::pointExtrapToCovered3D: vof " << a_vof << " drops order " << endl;
          a_extrapVal[ivar] = 0.;
          Real volTot = 0.;
          bool hasAVoF = false;
          for(int ix = 0; ix < 2; ix++)
            ...
            for(int ivar = 0; ivar < a_consState.nComp(); ivar++)
              {
                ...
                //but got replaced in uncache
                Real originalState= a_consState(vof, ivar);
          ...
          for(int ivar = 0; ivar < numCons; ivar++)
            {
              ...
              Real deltaF = -coeff*abs(dv)*(uhi-ulo);
