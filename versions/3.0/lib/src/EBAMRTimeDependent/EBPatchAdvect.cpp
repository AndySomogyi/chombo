        ...
        const Box&       a_box)
{
  ...
  IntVectSet ivsMulti = a_primGdnv.getMultiCells();
  ivsMulti &= cellBox;
  
  FaceStop::WhichFaces stopCrit;
  if(m_domain.isPeriodic(a_faceDir))
    {
  ...
  for(FaceIterator faceit(ivsMulti, m_ebisBox.getEBGraph(), a_faceDir, stopCrit);
      faceit.ok(); ++faceit)
    {
      const FaceIndex& face = faceit();
      VolIndex vofl = face.getVoF(Side::Lo);
      VolIndex vofr = face.getVoF(Side::Hi);
      Real velHi = normalVel(vofr, a_faceDir);
      Real velLo = normalVel(vofl, a_faceDir);
      for(int ivar = 0; ivar < nPrim; ivar++)
        {
          if((velHi > 0.0) && (velLo > 0.0))
            {
              a_primGdnv(face, ivar) = a_primLeft(vofr, ivar);
            }
          else if((velHi < 0.0) && (velLo < 0.0))
            {
              a_primGdnv(face, ivar) = a_primRigh(vofl, ivar);
            }
          else
            {
              if((s_doingVel == 1) && (s_curComp == a_faceDir))
                ...
                {
                  a_primGdnv(face, ivar) = 0.5*(a_primRigh(vofl, ivar) + a_primLeft(vofr, ivar));
                }
            }
        }
    }
