          ...
          if(a_phase2 != NULL)
            {
              ...
              // copy regular data
              currentFab.copy(ebcellfab2.getSingleValuedFAB(),0, ncomp, ncomp);
...
{ 
  ...
  const IntVectSet& ivsMulti = a_face->getMultiCells();
               ...
               const char*                           a_filename)
{
  ...
  Vector<ProblemDomain> amrDomain(numLevels);
           ...
           const IntVect& a_ghost)
{
  ...
  a_handle.open(a_filename, HDF5Handle::CREATE);
...
VolHDF5::VolHDF5()
{
  if(id == 0){
    VolHDF5& v = *this;
    //id = H5Tcreate(H5T_COMPOUND, 3*sizeof(RealVect)+2*sizeof(Real) + sizeof(IntVect));
    ...
    H5Tinsert (id, "cell",   CHOFFSET(v, cell), HDF5Handle::intvect_id);
  }
}
...
FaceHDF5::FaceHDF5()
{
  if(id == 0){
    FaceHDF5& f = *this;
    //id = H5Tcreate(H5T_COMPOUND, sizeof(RealVect) + sizeof(Real) + 2*sizeof(int));
    id = H5Tcreate(H5T_COMPOUND, sizeof(FaceHDF5));
    H5Tinsert (id, "areaFrac",  CHOFFSET(f, areaFrac),     H5T_NATIVE_REAL);
    H5Tinsert (id, "centroid",  CHOFFSET(f, centroid), HDF5Handle::realvect_id);
    H5Tinsert (id, "hiVof",     CHOFFSET(f, hiVof),  H5T_NATIVE_INT);
    H5Tinsert (id, "loVof",     CHOFFSET(f, loVof),  H5T_NATIVE_INT);
                 ...
                 Vector<Vector<long long> >& a_offsets)
{
  ...
  char names[4][9] = {{"VOffsets"}, {"XOffsets"}, {"YOffsets"}, {"ZOffsets"}};
            ...
            Vector<Vector<long long> >& a_offsets)
{
  ...
  createData(vdataset, vdataspace, a_handle, std::string("VOFs"),
                VolHDF5::id, (hsize_t)a_offsets[0].back());
        ...
        for(int dir=0; dir<CH_SPACEDIM; ++dir){
          ...
          if(size > 0){
            ...
            for(int i=0; i<size; i++){
              ...
              fbuffer[i].areaFrac = ebox.areaFrac(face);
              fbuffer[i].centroid = ebox.centroid(face);
              fbuffer[i].hiVof = (int)(vdata(face.getVoF(Side::Hi),0).m_averageFace.m_volIndex.cellIndex());
              fbuffer[i].loVof = (int)(vdata(face.getVoF(Side::Lo),0).m_averageFace.m_volIndex.cellIndex());
            }
  ...
  // Now write out covered/uncovered/irregular
  ...
  for(DataIterator dit = a_data->dataIterator(); dit.ok(); ++dit)
    {
      ...
      m.setVal(-1);
      if(graph.isAllRegular())
        {
          m.setVal(1);
        }
      else if(graph.isAllCovered())
        {
          m.setVal(0);
        }
      else
                  ...
                  Interval a_interval)
{
  ...
  write(a_handle, a_data->disjointBoxLayout(), "Boxes");
  ...
  createDataset(dataset, dataspace, a_handle, "CIrregular",
                dummy, ncomps*offsets[offsets.size()-1]);
