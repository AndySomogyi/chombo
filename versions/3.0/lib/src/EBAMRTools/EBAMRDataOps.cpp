  ...
  for(int ilev = 0; ilev < numLevels; ilev++)
    {
      ...
      numBox     = EBLevelDataOps::parallelSum(numBox);
      numRegIV   = EBLevelDataOps::parallelSum(numRegIV);
      numRegInIV = EBLevelDataOps::parallelSum(numRegInIV);
      numIrreg   = EBLevelDataOps::parallelSum(numIrreg);
      numMulti   = EBLevelDataOps::parallelSum(numMulti);
      numVofs    = EBLevelDataOps::parallelSum(numVofs);

      pout() << "     Stats for Level " << ilev << ":" << endl;
      pout() << "       Domain = " << a_domain[ilev] << endl;
      pout() << "       The number of               boxes = " << numBox     << endl;
      pout() << "       The number of        regular iv's = " << numRegIV   << endl;
      pout() << "       The number of  uncov regular iv's = " << numRegInIV << endl;
      pout() << "       The number of      irregular iv's = " << numIrreg   << endl;
      pout() << "       The number of          multi vofs = " << numMulti   << endl;
      pout() << "       The number of                vofs = " << numVofs    << endl;
      pout() << "       The number of        domain cells = " << numDomIV   << endl;
#ifdef CH_MPI
      ...
      numBoxAMR     += numBox;
      numRegIVAMR   += numRegIV;
      numRegInIVAMR += numRegInIV;
      numIrregAMR   += numIrreg;
      numMultiAMR   += numMulti;
      numVofsAMR    += numVofs;
    }
...
void EBAMRDataOps::pwqFillPatchAll(Vector<LevelData<EBCellFAB>* >&  a_data,
                                   const Vector<EBLevelGrid>&       a_eblg,
                                   const Vector<int>&               a_refRatio)
{
  coarsenDown(a_data,a_eblg,a_refRatio);
  int numLevels = a_data.size();
  for(int ilev = 1; ilev < numLevels; ilev++)
    {
      pwqFillPatchOne(*a_data[ilev],
                      *a_data[ilev-1],
                      a_eblg[ilev],
                      a_eblg[ilev-1],
                      a_refRatio[ilev-1]);
...
void EBAMRDataOps::pwqFillPatchOne(LevelData<EBCellFAB>&       a_dataFine,
                                   LevelData<EBCellFAB>&       a_dataCoar,
                                   const EBLevelGrid&          a_eblgFine,
                                   const EBLevelGrid&          a_eblgCoar,
                                   const int&                  a_refRatioCoar)
{//make sure you have coarsened down!!
  CH_TIME("EBAMRDataOps::pwqFillPatchOne");
  int nComp = a_dataCoar.nComp();
  IntVect ghostVect = a_dataCoar.ghostVect();

  Interval  intval(0,nComp-1);
  EBPWQuadFillPatch patcher(a_eblgFine.getDBL(),
                            a_eblgCoar.getDBL(),
                            a_eblgFine.getEBISL(),
                            a_eblgCoar.getEBISL(),
