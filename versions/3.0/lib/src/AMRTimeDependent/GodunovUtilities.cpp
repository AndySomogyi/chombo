...
//////////////////////////////////////////////////////////////////////////////
void GodunovUtilities::PPMFaceValuesunused(FArrayBox&            a_WFace,
                                     const FArrayBox&      a_W,
                                     const int&            a_numSlopes,
                                     const bool&           a_useLimiting,
                                     const int&            a_dir,
                                     ...
                                     const GodunovPhysics* a_physPtr)
{
  ...
  Box vlBox = a_box;
  vlBox.enclosedCells();
  // petermc, 17 Sep 2008, changed from 1 to 2 in high-order case
  int ghostvlBox = 1; // FIX, 19 Sep 2008 (m_highOrderLimiter) ? 2 : 1;
  // int ghostvlBox = (m_highOrderLimiter) ? 2 : 1;
  vlBox.grow(a_dir, ghostvlBox);
  ...
  // For i in centerBox,
  // a_Wface[i-e/2] = (a_W[i-e] + a_W[i])/2 + (dW[i-e] + dW[i])/6
  //                = (a_W[i-e] + dW[i-e]/3)/2 + (a_W[i] - dW[i]/3)/2.
  // For i in loBox, a_Wface[i-e/2] = a_W[i]- dW[i]/2.
  // For i in hiBox, a_Wface[i-e/2] = a_W[i-e] + dW[i-e]/2.
  // We calculate a_Wface on centerBox,
  // and we need a_W and dW on centerBox including shift down by 1 in direction a_dir.
  ...
  if (m_highOrderLimiter)
    {
      // Compute where centered differences can be used and where one-sided
      // differences need to be used.
      Box loCells, hiCells, centerCells, entireCells;
      int hasLoCells, hasHiCells;
      loHiCenter(loCells, hasLoCells, hiCells, hasHiCells,
                 centerCells, entireCells,
                 box1cells, m_domain, a_dir);

      // Added by petermc, 10 Dec 2008
      Box box0cells = a_box;
      box0cells.enclosedCells();
      ...
      // If requested, apply the limiter to faces.
      if (a_useLimiting)
        {
          // loFaces and hiFaces are ON domain boundary; do nothing there.
          
          // centerFaces includes faces that are 1 away from boundary:
          // there we are missing one of the second derivatives from
          // cell averages, but d2Wcell will have a copy of the one we have.

          // For i in centerCells, set d2WcellAvg[i] from a_W[i-e:i+e].
          // For i in loCells, set d2WcellAvg[i] = d2WcellAvg[i+e].
          // For i in hiCells, set d2WcellAvg[i] = d2WcellAvg[i-e].
          FArrayBox d2WcellAvg(entireCells, a_numSlopes);
          FORT_GETD2CELL(CHF_FRA(d2WcellAvg),
                         CHF_CONST_FRA(a_W),
                         CHF_CONST_INT(a_numSlopes),
                         CHF_CONST_INT(a_dir),
                         CHF_BOX(loCells),
          ...
          // For i-e/2 in centerFaces, apply limiter to a_WFace[i-e/2],
          // using a_W[i-e], a_W[i], and d2Wlim[i-e/2].
          Real limitC = 1.25;
          FORT_COLELLASEKORAFACELIMITER(CHF_FRA(a_WFace),
                                        CHF_CONST_FRA(a_W),
                                        CHF_CONST_FRA(d2WcellAvg),
                                        CHF_CONST_INT(a_numSlopes),
                                        CHF_CONST_INT(a_dir),
                                        CHF_BOX(centerFaces),
                                  ...
                                  const Box& a_box)
{
  ...
  // a_dWMinus[i] = WFace[i-e_d/2] - a_W[i]
  // a_dWPlus[i] = WFace[i+e_d/2] - a_W[i]
  ...
  if (m_highOrderLimiter)
    {
      ...
      // We calculate d2W on a_box grown by 1 in dimension a_dir,
      // and we need a_W on a_box grown by 2 in dimension a_dir.
      ...
      Real limitC = 1.25;
      Real eps = 1.0e-12;
      // In the case of centerBox == entireBox == a_box:
      // We modify a_dWMinus and a_dWMinus on centerBox,
      // and we need a_W on a_box grown by 1 in direction a_dir,
      // and we need d2Wfcf on a_box.
      FORT_COLELLASEKORALIMITERF(// <W>_{i-1/2} - <W>_i, on a_box
                                 CHF_FRA(a_dWMinus), 
