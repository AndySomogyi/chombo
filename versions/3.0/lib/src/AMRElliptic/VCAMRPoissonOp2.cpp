...
void VCAMRPoissonOp2::relax(LevelData<FArrayBox>&       a_e,
                           const LevelData<FArrayBox>& a_residual,
                           int                         a_iterations)
{
  CH_TIME("VCAMRPoissonOp2::relax");
...
{
  CH_TIMERS("VCAMRPoissonOp::reflux");

  m_levfluxreg.setToZero();
  Interval interv(0,a_phi.nComp()-1);

  CH_TIMER("VCAMRPoissonOp::reflux::incrementCoarse", t2);
  CH_START(t2);
  ...
  CH_TIMER("VCAMRPoissonOp::reflux::incrementFine", t3);
  CH_START(t3);
  ...
  for (int whichPass = 0; whichPass <= 1; whichPass++)
    {
      ...
      {
        CH_TIME("VCAMRPoissonOp::levelGSRB::BCs");
                               ...
                               const LevelData<FArrayBox>& a_rhs)
{
  ...
  for (dit.begin(); dit.ok(); ++dit)
  {
    // invoke physical BC's where necessary
    {
      CH_TIME("VCAMRPoissonOp::looseGSRB::BCs");
      m_bc(a_phi[dit], dbl[dit()], m_domain, m_dx, true);
    }
