...

{
  ...
  kappaDivergence(m_divu, a_velocity, a_boundaryVelo, doDivFreeOutflow);
  ...
  m_solver.solveNoInit(m_phi, m_divu, m_numLevels-1, 0);
  ...
  //  this helps EBCompositeCCProjector during the extrap to domain faces step)
  ...
  //for extrapolation-type boundary conditions, need to reenforce bcs
  doDivFreeOutflow = false;
  enforceVelocityBC(a_velocity,doDivFreeOutflow);
      ...
      for(SideIterator sitVel; sitVel.ok(); ++sitVel)
        {
          ...
          if(!a_cfivs.contains(ivSide) && domainBox.contains(ivSide))
            ...
            {
              // inside coarse/fine interface or at domain boundary. extrapolate from neighboring vofs to get the
              //gradient
                ...
                const bool&                                  a_doDivFreeOutflow)
{
  ...
  enforceVelocityBC(a_velo,a_doDivFreeOutflow);//returns face centered values
  for(int ilev = 0; ilev < m_numLevels; ilev++)
    {
      EBLevelMACProjector::setCurLevel(ilev);
      ...
      //compute the divergence igorning other levels
      ...
      if(ilev < (m_numLevels-1))
        {
          ...
          for(int idir = 0; idir < SpaceDim; idir++)
            {
              Real reflScale = 1.0/m_dx[ilev][idir];
              m_fluxReg[ilev]->reflux(*a_divu[ilev], interv, reflScale,idir);
            }
        }
    }
}
