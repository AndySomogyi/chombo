...
bool EBAMRPoissonOp::s_doTrimEdges = true;
      ...
      if(numPtsLeft == 0)
         {
          ...
          Vector<int> procs;
//           EBLoadBalance(procs, boxes ,a_domainCoar);
          if(s_doEBEllipticLoadBalance)
            {
              EBEllipticLoadBalance(procs, boxes, a_domainCoar);
            }
          else
            {
          ...
          for(int iface = 0; iface < faces.size(); iface++)
            {
              ...
              getFluxStencil(fluxStencil, faces[iface], a_dit, a_doFaceInterp);
               ...
               bool             a_doFaceInterp)
{
  ...
  if(a_doFaceInterp)
    ...
    {
      a_fluxStencil.clear();
      getFaceCenteredFluxStencil(a_fluxStencil, a_face, a_dit);
    }
                                ...
                                const Real& a_beta)
{
  ...
  m_amrpop.m_alpha = m_alpha;
  m_amrpop.m_beta = m_beta;
  m_amrpop.m_aCoef = 1;
  m_amrpop.m_bCoef = 1;
}
void
    ...
    m_ghostCellsRHS( a_ghostCellsRHS )
{
  ...
  if(s_doTrimEdges) {m_exchangeCopier.trimEdges(m_eblg.getDBL(), m_ghostCellsPhi);}

  m_cfregion.define(m_eblg.getDBL(), m_eblg.getDomain());
  BCHolder bcfunc;
  if(a_hasFine && a_hasCoar)
    {
      m_amrpop.define(m_eblg.getDBL(), m_eblgFine.getDBL(), m_eblgCoar.getDBL(),
                      m_dx[0], m_refToCoar, m_refToFine,
                      m_eblg.getDomain(), bcfunc, m_exchangeCopier, m_cfregion);
    }
  else if(a_hasCoar)
    {
      m_amrpop.define(m_eblg.getDBL(), m_eblgCoar.getDBL(), m_dx[0], m_refToCoar,
                      m_eblg.getDomain(), bcfunc, m_exchangeCopier, m_cfregion);

    }
  else if(a_hasFine)
    {
      //bcs do not mattter.   this is just used for refluxing
      m_amrpop.define(m_eblg.getDBL(), m_eblgFine.getDBL(), m_dx[0], 1, m_refToFine,
                      m_eblg.getDomain(), bcfunc, m_exchangeCopier, m_cfregion);
    }
  else
    {
      //bcs do not mattter.   this is just used for refluxing
      m_amrpop.define(m_eblg.getDBL(), m_dx[0],
                      m_eblg.getDomain(), bcfunc, m_exchangeCopier, m_cfregion);
    }
  m_amrpop.m_alpha = m_alpha;
  m_amrpop.m_beta =  m_beta;
...
defineStencils()
{
  ...
  //define bc stencils and create flux stencil, beta stays out
  //and gets multipleid in later, so does boundary area and dx
  m_ebBC->define((*m_eblg.getCFIVS()), m_dxScale);
  ...
  if (m_relaxType == 1 || m_relaxType == 2 || m_relaxType ==3)
    {
          ...
          if(fluxStencil != NULL)
            {
              ...
              const VoFStencil&  fluxStencilPt = fluxStencilBaseIVFAB(VoF,0);
              ...
              if(fluxStencilPt.size() == 0)
              ...
              else
                {
                  ...
                  relStencil += fluxStencilPt;
                }
            }
        }

      //Operator ebstencil
      m_opEBStencil[dit()] = RefCountedPtr<EBStencil>(new EBStencil(m_vofItIrreg[dit()].getVector(), opStencil[dit()], m_eblg.getDBL().get(dit()), m_eblg.getEBISL()[dit()], m_ghostCellsPhi, m_ghostCellsRHS, 0, true));
      ...
      for (DataIterator dit = m_eblg.getDBL().dataIterator(); dit.ok(); ++dit)
        {
          ...
          m_colorEBStencil[icolor][dit()]  = RefCountedPtr<EBStencil>(new EBStencil(srcVofs, colorStencil[dit()]   ,  m_eblg.getDBL().get(dit()), m_eblg.getEBISL()[dit()], m_ghostCellsPhi, m_ghostCellsRHS, 0, true));
        }//dit
    }//color
}
...
preCond(LevelData<EBCellFAB>&       a_lhs,
        const LevelData<EBCellFAB>& a_rhs)
{
  CH_TIME("EBAMRPoissonOp::preCond");
  ...
  for (DataIterator dit = phi.dataIterator(); dit.ok(); ++dit)
    {
      ...
      Box dblBox( m_eblg.getDBL().get(dit()) );
//       m_domainBC->setCurrentBox(dblBox);
      const EBCellFAB& curPhiEBCellFAB = phi[dit()];
      Box curPhiBox = curPhiEBCellFAB.box();
      const BaseFab<Real>& curPhiFAB = curPhiEBCellFAB.getSingleValuedFAB();

      EBCellFAB& curOpPhiEBCellFAB = a_opPhi[dit()];
      ...
      m_opEBStencil[dit()]->apply(curOpPhiEBCellFAB, curPhiEBCellFAB, alphaWeight, m_alpha, m_beta, false);
      CH_STOP(t2);
      ...
      if(hasNoEBLevelData)
          ...
          if(!a_homogeneousPhysBC)
            {
              ...
              m_ebBC->applyEBFlux(curOpPhiEBCellFAB, curPhiEBCellFAB, m_vofItIrreg[dit()], (*m_eblg.getCFIVS()),
      ...
      else
        {
          ...
          for(vofit.reset(); vofit.ok(); ++vofit)
            {
              const VolIndex& vof = vofit();
              const BaseIVFAB<Real>& ebInputFluxFAB = (*a_ebFluxBCLD)[dit()];
              ...
              for(m_vofItIrregDomLo[idir][dit()].reset(); m_vofItIrregDomLo[idir][dit()].ok();  ++m_vofItIrregDomLo[idir][dit()])
                {
                  ...
                  curOpPhiEBCellFAB(vof,comp) -= flux * m_beta*m_invDx[idir];
                }
 ...
 for(DataIterator dit = a_rhs.dataIterator(); dit.ok(); ++dit)
   {
     ...
     const BaseFab<int>& maskFAB = m_eblg.getEBISL()[dit()].getEBGraph().getMask(iRegIrregCovered);
       ...
       AMRLevelOp<LevelData<EBCellFAB> >* a_finerOp)
{
  ...
  ParmParse pp;
  int which_reflux = 0;
  pp.query("which_reflux",which_reflux);
  if(which_reflux == 0)
    {
      EBAMRPoissonOp* finerOp = (EBAMRPoissonOp*) a_finerOp;
      AMRPoissonOp* finerAMRPOP = &(finerOp->m_amrpop);
      LevelData<FArrayBox> phiFineLDFAB, phiLDFAB, residLDFAB;
      LevelData<EBCellFAB>& phif = (LevelData<EBCellFAB>&)(a_phiFine);
      LevelData<EBCellFAB>& phi  = (LevelData<EBCellFAB>&)(a_phi);
      aliasEB(phiFineLDFAB, phif);
      aliasEB(phiLDFAB,     phi);
      aliasEB(residLDFAB,   a_residual);
      AMRLevelOp<LevelData<FArrayBox> >*  castOp = (AMRLevelOp<LevelData<FArrayBox> >*)(finerAMRPOP);
      m_amrpop.setAlphaAndBeta(m_alpha, m_beta);
      m_amrpop.reflux(phiFineLDFAB, phiLDFAB, residLDFAB, castOp);
    }
  else if(which_reflux == 1)
    {
      this->old_reflux( a_residual, a_phiFine, a_phi, a_finerOp);
    }
  else if(which_reflux == 2)
    {
      this->fast_reflux(a_residual, a_phiFine, a_phi, a_finerOp);
    }
  else
    {
      MayDay::Error("bogus which_reflux");
    }
}

void EBAMRPoissonOp::
old_reflux(LevelData<EBCellFAB>& a_residual,
           const LevelData<EBCellFAB>& a_phiFine,
           const LevelData<EBCellFAB>& a_phi,
           AMRLevelOp<LevelData<EBCellFAB> >* a_finerOp)
{
  CH_TIMERS("EBAMRPoissonOp::OldReflux");
  CH_TIMER("define",t1);
  ...
  CH_STOP(t2);
  CH_START(t3);
  old_incrementFRCoar(m_fluxReg, a_phiFine, a_phi);
  CH_STOP(t3);

  CH_START(t4);
  old_incrementFRFine(m_fluxReg, a_phiFine, a_phi, a_finerOp);
  CH_STOP(t4);
  CH_START(t5);
  for(int idir = 0; idir < SpaceDim; idir++)
    {
      Real scale = 1.0/m_dx[idir];
            ...
            AMRLevelOp<LevelData<EBCellFAB> >* a_finerOp)
{
  ...
  CH_STOP(t2);
  CH_START(t3);
  fast_incrementFRCoar(m_fastFR, a_phiFine, a_phi);
  CH_STOP(t3);

  CH_START(t4);
  fast_incrementFRFine(m_fastFR, a_phiFine, a_phi, a_finerOp);
  CH_STOP(t4);
  CH_START(t5);
...
/****/
void EBAMRPoissonOp::
old_incrementFRCoar(EBFluxRegister&             a_fluxReg,
                const LevelData<EBCellFAB>& a_phiFine,
                const LevelData<EBCellFAB>& a_phi)
{
      ...
      for (int idir = 0; idir < SpaceDim; idir++)
        {
          ...
          Real scale = 1.0;
          a_fluxReg.incrementCoarseBoth(coarflux, scale, dit(), interv, idir);
        }
    }
}

void EBAMRPoissonOp::
fast_incrementFRCoar(EBFastFR&             a_fluxReg,
                     const LevelData<EBCellFAB>& a_phiFine,
                     const LevelData<EBCellFAB>& a_phi)
{
  CH_TIME("EBAMRPoissonOp::incrementFRCoar");
...
/***/
void EBAMRPoissonOp::
old_incrementFRFine(EBFluxRegister&             a_fluxReg,
                const LevelData<EBCellFAB>& a_phiFine,
                const LevelData<EBCellFAB>& a_phi,
                AMRLevelOp<LevelData<EBCellFAB> >* a_finerOp)
{
          ...
          for (sit.begin(); sit.ok(); sit.next())
            {
              ...
              getFlux(fluxFine, phiFine, ghostedBox, fabBox, m_eblgFine.getDomain(),
                      ebisBoxFine, m_dxFine, idir);
              ...
              a_fluxReg.incrementFineBoth(fluxFine, scale, ditf(),
                                          interv,  idir,    sit());
            }
        }
    }
...
/***/
void EBAMRPoissonOp::
fast_incrementFRFine(EBFastFR&             a_fluxReg,
                const LevelData<EBCellFAB>& a_phiFine,
                const LevelData<EBCellFAB>& a_phi,
                AMRLevelOp<LevelData<EBCellFAB> >* a_finerOp)
{
  ...
  for(DataIterator dit = dbl.dataIterator(); dit.ok(); ++dit)
    {
      ...
      for(VoFIterator vofit(irregIVS, ebgraph); vofit.ok(); ++vofit)
        {
          Real alphaWeight  = m_alphaDiagWeight[dit()](vofit(),0);
          Real  betaWeight  =  m_betaDiagWeight[dit()](vofit(),0);
          Real weightIrreg = m_alpha*alphaWeight + m_beta*betaWeight;
          relCoef[dit()](vofit(),0) = safety/weightIrreg;
        }
    }
                    ...
                    const DataIndex&             a_dit)
{
  ...
  // assume EBFlux leveldata is null, i.e., we have no input eb flux
          ...
          for(m_vofItIrregColorDomLo[a_icolor][idir][a_dit].reset(); m_vofItIrregColorDomLo[a_icolor][idir][a_dit].ok();  ++m_vofItIrregColorDomLo[a_icolor][idir][a_dit])
            {
              ...
              m_domainBC->getFaceFlux(flux,vof,comp,a_phi,
                                      m_origin,m_dx,idir,Side::Lo, a_dit, s_time,
                                      a_homogeneousPhysBC);

              Real phiOld = a_phi(vof,comp);
              Real phiNew = phiOld + (1./weightIrreg) * flux * m_beta*m_invDx[idir];
              a_phi(vof,comp) = phiNew;
            }
          CH_STOP(t2);
          CH_START(t3);
          for(m_vofItIrregColorDomHi[a_icolor][idir][a_dit].reset(); m_vofItIrregColorDomHi[a_icolor][idir][a_dit].ok();  ++m_vofItIrregColorDomHi[a_icolor][idir][a_dit])
            {
              ...
              Real weightIrreg = m_alpha*curAlphaWeight(vof,0) + m_beta*curBetaWeight(vof,0);
              m_domainBC->getFaceFlux(flux,vof,comp,a_phi,
                                      m_origin,m_dx,idir,Side::Hi,a_dit,s_time,
                                      a_homogeneousPhysBC);
              ...
              for(int icomp=0; icomp<ncomp; icomp++)
                {
                  ...
                  Real increment = (1.0/weightIrreg)*residVal;
