      ...
      for(int j=i+1; j<CH_SPACEDIM; ++j)
        {
          ...
          if(lo.intersects(loj)) a_overlap.push_back(lo & loj);
          if(lo.intersects(hij)) a_overlap.push_back(lo & hij);
        }
    }
    if(!hi.isEmpty()){
      for(int j=i+1; j<CH_SPACEDIM; ++j)
        {
          const Box& loj = a_loBox[j];
          const Box& hij = a_hiBox[j];
          if(hi.intersects(loj)) a_overlap.push_back(hi & loj);
          if(hi.intersects(hij)) a_overlap.push_back(hi & hij);
        }
    }
  }
}
