...
defineStencils()
{
  ...
  // define ebstencil for level Jacobi
  m_jacobiEBStencil.define(m_eblg.getDBL());
  m_rhsJacobiEBStencil.define(m_eblg.getDBL());
  // create vofstencils for applyOp and Jacobi
  LayoutData<BaseIVFAB<VoFStencil> > opStencil;
  opStencil.define(m_eblg.getDBL());
  LayoutData<BaseIVFAB<VoFStencil> > jacobiStencil;
  jacobiStencil.define(m_eblg.getDBL());
  LayoutData<BaseIVFAB<VoFStencil> > rhsJacobiStencil;
  rhsJacobiStencil.define(m_eblg.getDBL());
  ...
  for (DataIterator dit = m_eblg.getDBL().dataIterator(); dit.ok(); ++dit)
    {
      ...
      BaseIVFAB<VoFStencil>& jacobiStencilBaseIVFAB = jacobiStencil[dit()];
      BaseIVFAB<VoFStencil>& rhsJacobiStencilBaseIVFAB = rhsJacobiStencil[dit()];
      ...
      jacobiStencilBaseIVFAB.define(notRegular,curEBGraph, 1);
      rhsJacobiStencilBaseIVFAB.define(notRegular,curEBGraph, 1);
      ...
      for(vofit.reset(); vofit.ok(); ++vofit)
        {
          ...
          VoFStencil& curStencil = curStencilBaseIVFAB(vof,0);
          VoFStencil& jacobiStencil = jacobiStencilBaseIVFAB(vof,0);
          VoFStencil& rhsJacobiStencil = rhsJacobiStencilBaseIVFAB(vof,0);

          Real& curAlphaWeight  = alphaWeight(vof,0);
          Real& curBetaWeight  =   betaWeight(vof,0);
          Real weightIrreg = m_alpha*curAlphaWeight + m_beta*curBetaWeight;

          jacobiStencil = curStencil;
          jacobiStencil *= (0.5/weightIrreg);
          jacobiStencil.add(vof, 1.0);
          rhsJacobiStencil.add(vof, 0.5/weightIrreg);
        }

      //Jacobi ebstencil
      m_jacobiEBStencil[dit()]       = RefCountedPtr<EBStencil>(new EBStencil(m_vofItIrreg[dit()].getVector(), jacobiStencil[dit()]        ,  m_eblg.getDBL().get(dit()), m_eblg.getEBISL()[dit()], m_ghostCellsPhi, m_ghostCellsPhi));
      m_rhsJacobiEBStencil[dit()]    = RefCountedPtr<EBStencil>(new EBStencil(m_vofItIrreg[dit()].getVector(), rhsJacobiStencil[dit()]     , m_eblg.getDBL().get(dit()) , m_eblg.getEBISL()[dit()], m_ghostCellsRHS, m_ghostCellsPhi));
  ...
  for (DataIterator dit = m_eblg.getDBL().dataIterator(); dit.ok(); ++dit)
    {
      ...
      //save phi in irreg cells
      m_jacobiEBStencil[dit()]->cache(phifab);
      ...
      for(int comp=0; comp<a_phi.nComp(); comp++)
        {
          FORT_DOALLREGULARJACOBI(CHF_FRA1(phiFAB,0),
      ...
      //recall phi in irreg cells
      m_jacobiEBStencil[dit()]->uncache(phifab);

      //Do the irregular cells

      //phi = (I-lambda*L)phi with homogeneous eb bcs
      m_jacobiEBStencil[dit()]->apply(phifab, phifab, false);
          ...
          for(m_vofItIrregDomLo[idir][dit()].reset(); m_vofItIrregDomLo[idir][dit()].ok();  ++m_vofItIrregDomLo[idir][dit()])
            {
              ...
              const VolIndex& vof = m_vofItIrregDomLo[idir][dit()]();
              Real weightIrreg = m_alpha*curAlphaWeight(vof,0) + m_beta*curBetaWeight(vof,0);
              m_domainBC->getFaceFlux(flux,vof,comp,phifab,
                                      m_origin,m_dx,idir,Side::Lo, dit(), m_time,
                                      homogeneous);

              phifab(vof,comp) -= (0.5/weightIrreg) * flux * m_beta*m_invDx[idir];
            }
          for(m_vofItIrregDomHi[idir][dit()].reset(); m_vofItIrregDomHi[idir][dit()].ok();  ++m_vofItIrregDomHi[idir][dit()])
            {
              Real flux;
      ...
      //phi += lambda*rhs
      m_rhsJacobiEBStencil[dit()]->apply(phifab, rhsfab, true);
