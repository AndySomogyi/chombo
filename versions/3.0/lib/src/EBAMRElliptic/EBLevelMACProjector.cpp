                    ...
                    const IntVect&                                  a_nghostRhs)
{
  ...
  int numPreCondIters = 40;//40 seems way too big, but that was the default...(try 4)
  int mgRelaxType = 1;
        ...
        const LevelData<BaseIVFAB<Real> >* const a_boundaryVelocity)
{
  ...
  //enforce boundary conditons (returns face centered values)
  bool doDivFreeOutflow = false;
  macEnforceVelocityBC(a_velocity, m_grids, m_ebisl, m_domain, m_dx, m_origin,m_domainBCFactVel,1.e99,doDivFreeOutflow,a_boundaryVelocity);
  ...
  doDivFreeOutflow = true;
  macEnforceVelocityBC(centroidVelocity, m_grids, m_ebisl, m_domain, m_dx, m_origin,m_domainBCFactVel,1.e99,doDivFreeOutflow,a_boundaryVelocity);
}
/*****/
void
EBLevelMACProjector::
                   ...
                   const BaseIVFAB<Real>* a_boundaryVel)
{
  ...
  Real dxScale;
  {
    ...
    dxScale = pow(maxDx,SpaceDim-1)/dv;
  }
          ...
          for(int idir = 0; idir < SpaceDim; idir++)
            {
              ...
              boundaryFlux += boundaryArea*normal[idir]*boundaryVel;
            }
          update -= boundaryFlux*dxScale;
        }
  ...
  // begin debug
  VolIndex vofLo(f_ivLo, 0);
  VolIndex vofHi(f_ivHi, 0);
  FaceIndex faceInt(vofLo, vofHi, 1);

  bool verbose = EBLevelMACProjector::getVerbose();
  int curLevel = EBLevelMACProjector::getCurLevel();
  string debugString = EBLevelMACProjector::getDebugString();
  if( verbose && ( curLevel ==2) && (a_box.contains(f_ivLo)  || a_box.contains(f_ivHi)))
    {
      pout() << setprecision(8)
             << setiosflags(ios::showpoint)
             << setiosflags(ios::scientific);

      pout() << debugString << " vel(faceInt) = "  << a_velo[1](faceInt, 0) << endl;
      if(a_box.contains(f_ivLo))
        {
          pout() << debugString << " divVel(vofLo) = "  <<  a_divVel(vofLo, 0) << endl;
        }
      if(a_box.contains(f_ivHi))
        {
          pout() << debugString << " divVel(vofHi) = "  <<  a_divVel(vofHi, 0) << endl;
        }
...

{
  ...
  domainBC->enforceFaceVel(a_velocity,a_grids,a_ebisl,a_domain,a_dx,a_time,a_origin,false);
  delete domainBC;
}
