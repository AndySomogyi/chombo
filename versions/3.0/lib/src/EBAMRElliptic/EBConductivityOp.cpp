...
{
  CH_TIME("EBConductivityOp::kappaWeight");
...
defineStencils()
{
  ...
  m_ebBC->setCoef(    m_eblg,   m_beta,      m_bcoIrreg);
      ...
      for(vofit.reset(); vofit.ok(); ++vofit)
        {
              ...
              if (loSide.contains(iv))
                {
                  ...
                  adjust += -weightedAreaFrac * m_beta/(m_dx*m_dx);
                }
          ...
          curStencil.add(VoF, alphaWeight);
                 ...
                 const DataIndex&       a_datInd)
{
  ...
  m_opEBStencil[a_datInd]->apply(a_lhs, a_phi, false);

  if(!a_homogeneous)
    {
      const Real factor = 1./m_dx; //beta and bcoef handled within applyEBFlux
...
relax(LevelData<EBCellFAB>&       a_phi,
      ...
      int                         a_iterations)
{
  CH_TIME("EBConductivityOp::relax");
      ...
      for (int idir = 0; idir < SpaceDim; idir++)
        {
          ...
          FArrayBox const& regPhi  = (FArrayBox&)( coarfab.getSingleValuedFAB());
          getFlux(regFlux, regPhi, faceBox, idir,  m_dx, dit());
  ...
  for(int idir = 0; idir < SpaceDim; idir++)
    {
      ...
      //this function is a tga thing so beta has to be taken care of somehow.
      a_flux[idir] *= m_beta; 
    }
}
/***/
          ...
          for (sit.begin(); sit.ok(); sit.next())
            {
              ...
              EBFaceFAB coarflux(ebisBoxFine, ghostedBox, idir, ncomp);
              finerEBAMROp.getFlux(coarflux, phiFine, ghostedBox, boxFine, m_eblgFine.getDomain(), ebisBoxFine, m_dxFine, ditf(), idir);

              Real scale = 1.0;
              for(SideIterator sit; sit.ok(); ++sit)
                {
                  a_fluxReg.incrementFineBoth(coarflux, scale, ditf(), interv, idir, sit());
                }
            }
        }
    }
