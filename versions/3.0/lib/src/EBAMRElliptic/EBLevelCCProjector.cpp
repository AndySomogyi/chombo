          ...
          for(FaceIterator faceit(sideIVS, a_ebGraph, a_idir, stopCritSide);  faceit.ok(); ++faceit)
            {
              ...
              Real extrapValue = EBArith::extrapToDomainFace(bndryFace,sit(),a_idir,a_ebGraph,a_faceData,0);
                          ...
                          const int&              a_icomp)
{
  ...
  //   based on the different attempts (normal and tangengtial from above)
  Real extrapValCov;
