        ...
        bool                 a_homoOnly)
{
  ...
  //
  EBConductivityOp* newOp = new EBConductivityOp(EBLevelGrid(), eblgMGLevel, EBLevelGrid(), eblgCoarMG, quadCFI,
                                                 dombc, ebbc, dxMGLevel, dxCoar, bogRef, bogRef, hasFine, hasCoar,
                                                 hasCoarMGObjects, layoutChanged, m_alpha, m_beta,
                                                 acoef, bcoef, bcoefIrreg, m_ghostCellsPhi, m_ghostCellsRhs);
  return newOp;
...
AMRnewOp(const ProblemDomain& a_domainFine)
{
  ...
  bool layoutChanged = true;
  EBConductivityOp* newOp = new EBConductivityOp(eblgFine, eblgMGLevel, eblgCoar, eblgCoarMG, m_quadCFI[ref],
