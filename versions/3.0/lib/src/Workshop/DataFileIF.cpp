                       ...
                       const bool&                 a_inside)
{
  ...
  m_data = ReadData(m_noDataValue,cin,a_dataType,m_num);
                       ...
                       const bool&                 a_inside)
{
  ...
  m_data = ReadData(m_noDataValue,curFile,a_dataType,m_num);
...
DataFileIF::DataFileIF(const RefCountedPtr<FArrayBox> a_data,
...
{
  DataFileIF* dataFilePtr = new DataFileIF(m_data,
                                           m_noDataValue,
                                           m_num,
                                           m_spacing,
                                           m_origin,
...
RefCountedPtr<FArrayBox>
  DataFileIF::ReadData(Real&                       a_maxValue,
                       istream&                    a_file,
                       const DataFileIF::DataType& a_dataType,
                       const IntVect&              a_num)
{
  ...
  if (a_dataType == DataFileIF::ASCII)
  {
    ...
    while (state[SpaceDim-1] < a_num[SpaceDim-1])
    {
      ...
      // Read the next value and store the data
      a_file >> curInput;
  ...
  else if (a_dataType == DataFileIF::Binary)
  {
    ...
    MayDay::Abort("Reading binary data type not implemented");
  }
  else
  {
