                                                   ...
                                                   const Real&          a_dx) const
{
  ...
  if (m_implicitFunction->fastIntersection(a_region, a_domain, a_origin, a_dx))
  ...
  else
    {
      ...
      physCorner+=a_origin;
      Real firstValue =  m_implicitFunction->value(physCorner);
      if( firstValue < 0 )
        {
      ...
      for (; bit.ok(); ++bit)
        {
          ...
          if(functionValue == 0 || firstValue == 0)
            if(functionValue < 0 || firstValue < 0)  return GeometryService::Irregular;
...
GeometryShop::fillGraph(BaseFab<int>&       a_regIrregCovered,
                        ...
                        const Real&         a_dx) const
{
  CH_TIME("GeometryShop::fillGraph");
...
//       if(iv[0]==38 && iv[1]==52 && iv[2]==41)
//         {
//           pout() << "this is the cell" << endl;
//           pout() << "a_regIrregCovered " << a_regIrregCovered(iv, 0) << endl;
//         }
    }
...
//       if((volFrac != 0.) && (volFrac != 1.))
      if((volFrac < -eps || volFrac > eps) && (volFrac < 1.-eps || volFrac > 1.+eps))
        {
          isIrregNode = true;
//           if(ivsit()[0]==38 && ivsit()[1]==52 && ivsit()[2]==41)
//             {
//               pout() << "volFrac " << volFrac << endl;
//               pout() << "isIrregNode " << isIrregNode << endl;
//             }
        }
      else
        {
...
//               else if ((volFrac == 1.) && (loArc[faceDir].size() == 1) && (hiArc[faceDir].size() == 1))
              ...
              else if ((volFrac > 1.-eps && volFrac < 1.-eps) && (loArc[faceDir].size() == 1) && (hiArc[faceDir].size() == 1))
                {
                  for (int numFace = 0; numFace < loAreaFrac[faceDir].size(); numFace++)
                    {
//                       if (loAreaFrac[faceDir][numFace] == 1.)
                      if (loAreaFrac[faceDir][numFace] < 1.-eps && loAreaFrac[faceDir][numFace] > 1.+eps)
                        {
                          isIrregNode = false;
                        }
                      else
                  ...
                  for (int numFace = 0; numFace < hiAreaFrac[faceDir].size(); numFace++)
                    {
//                       if (hiAreaFrac[faceDir][numFace] == 1.)
                      if (hiAreaFrac[faceDir][numFace] < 1.-eps && hiAreaFrac[faceDir][numFace] > 1.+eps)
                        {
                          isIrregNode = false;
                        }
                      else
...
//           if(ivsit()[0]==38 && ivsit()[1]==52 && ivsit()[2]==41)
//             {
//               pout() << "add to list" << endl;
//               pout() << "a_regIrregCovered " << a_regIrregCovered(ivsit(), 0) << endl;
//             }
        }
      else
        {
//           if((volFrac == 0.)){a_regIrregCovered(ivsit(), 0) = -1;}
//           if((volFrac == 1.)){a_regIrregCovered(ivsit(), 0) =  1;}
          if((volFrac > -eps && volFrac < eps)){a_regIrregCovered(ivsit(), 0) = -1;}
          if((volFrac > 1.-eps && volFrac < 1.+eps)){a_regIrregCovered(ivsit(), 0) =  1;}
//           if(ivsit()[0]==38 && ivsit()[1]==52 && ivsit()[2]==41)
//             {
//               pout() << "take out of list" << endl;
//               pout() << "a_regIrregCovered " << a_regIrregCovered(ivsit(), 0) << endl;
//             }
        }
                                  ...
                                  const IntVect&      a_iv)const
{
  ...
  if (SpaceDim==3)
    {
      // 1) using the intvect,  build up the classes in Moments: edgeMO, faceMo and finally vofMo
      //2) check for covered or regular faces
      // 3) call  momentCalc3D
      // 4) keep track of what the output means and fill the variables asked for.
          ...
          if (crossingPt.size() == 2)
            {
              ...
              //loEdge-hiEdge
                ...
                {
                  MayDay::Error("cPtDir or mindir or maxDir not set correctly");
                }
              ...
              if (covered)
              ...
              else if (dontKnow)
                {
                  ...
                  if (funcHi >= 0 && funcLo*funcHi <= 0)
                    ...
                    {
                      MayDay::Error("Bogus intersection calculated");
                    }
                }
