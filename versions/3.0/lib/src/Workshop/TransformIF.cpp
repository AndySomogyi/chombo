...
IndexTM<Real,GLOBALDIM> TransformIF::normal(const IndexTM<Real,GLOBALDIM>& a_point) const
{
  // Function value at the inverse transform of a_point
  IndexTM<Real,GLOBALDIM> retval;
  IndexTM<Real,GLOBALDIM> retNormal;

  // The inverse transform of a_point
  IndexTM<Real,GLOBALDIM> invPoint;

  // Inverse transform a_point to invPoint
  vectorMultiply(invPoint,m_invTransform,a_point);

  // Get the function value at invPoint
  retval = m_impFunc->normal(invPoint);
  Real norm = 0.0;
  for (int i = 0; i < GLOBALDIM; i++)
  {
    ...
    {
      total += m_transform[i][k] * retval[k];
    }

    retNormal[i] = total;
    norm += retNormal[i] * retNormal[i];
  }

  norm = sqrt(norm);
  //the normal could be 0
  if (norm!=0.0)
    {
      retNormal /= norm;
    }
  //vectorMultiply(retNormal,m_transform,retval);

  return retNormal;
}

Vector< IndexTM<Real,GLOBALDIM> > TransformIF::gradNormal(const IndexTM<Real,GLOBALDIM>& a_point) const
{
  Vector< IndexTM<Real,GLOBALDIM> > gradNorm;
  Vector< IndexTM<Real,GLOBALDIM> > retGradNorm(GLOBALDIM);

  IndexTM<Real,GLOBALDIM> invPoint;
  vectorMultiply(invPoint,m_invTransform,a_point);

  gradNorm = m_impFunc->gradNormal(invPoint);
      ...
      for (int j = 0; j < GLOBALDIM; j++)
        {
          ...
          for (int k = 0; k < GLOBALDIM; k++)
            {
              total += m_transform[j][k] * gradNorm[i][k];
            }

          retGradNorm[i][j] = total;
          norm += retGradNorm[i][j] * retGradNorm[i][j];
        }
      norm = sqrt(norm);
      //the gradnormal could legitimately be 0
      if (norm!=0.0)
        {
          retGradNorm[i] /= norm;
        }
      //vectorMultiply(gradNorm[i],m_transform,gradNorm[i]);
    }
