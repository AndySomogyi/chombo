...
IndexTM<Real,GLOBALDIM> GaussianSphereIF::grad(const IndexTM<Real,GLOBALDIM>& a_point) const
{
  IndexTM<Real,GLOBALDIM> retval;

  // Function value
  Real curValue = value(a_point);

  // Compute the gradient
  for (int idir = 0; idir < GLOBALDIM; idir++)
  {
    retval[idir] = - (a_point[idir] - m_center[idir]) * curValue / m_sigma2;
  }

  return retval;
}

Vector<IndexTM<Real,GLOBALDIM> > GaussianSphereIF::gradGrad(const IndexTM<Real,GLOBALDIM>& a_point) const
{
  ...
  for (int idir = 0; idir < GLOBALDIM; idir++)
  {
    ...
    {
      retval[idir][jdir] = (a_point[idir] - m_center[idir]) * (a_point[jdir] - m_center[jdir]) * curValue / (m_sigma2 * m_sigma2);
    }
  }

  // Additional term in the repeated index terms
  for (int idir = 0; idir < GLOBALDIM; idir++)
  {
    retval[idir][idir] -= curValue / m_sigma2;
  }
