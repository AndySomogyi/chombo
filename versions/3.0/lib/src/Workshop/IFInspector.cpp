  ...
  for(VoFIterator vofit(ivs,ebisBox.getEBGraph()); vofit.ok(); ++vofit)
    {
      ...
      IndexTM<Real,GLOBALDIM> normal = a_imIF.normal(coord);
      for(int idir=0; idir < GLOBALDIM; ++idir)
        {
          a_ebFab(vof,idir)=normal[idir];
        }
    }
}
  ...
  for(VoFIterator vofit(ivs,ebisBox.getEBGraph()); vofit.ok(); ++vofit)
    {
      ...
      Vector<IndexTM <Real,GLOBALDIM> > gradNormal = a_imIF.gradNormal(coord);
      int k = 0;
      for(int idir=0; idir <GLOBALDIM ; ++idir)
        {
          for(int jdir=0; jdir <GLOBALDIM ; ++jdir)
            {
              a_ebFab(vof,k++)=gradNormal[idir][jdir];
            }
        }
