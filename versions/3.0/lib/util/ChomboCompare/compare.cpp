...
int main(int argc, char* argv[])
{
  ...
  init(exactRoot, computedRoot, errorRoot, intFieldSize,
       ...
       computeRelativeError, removeMean, doGhostCells, verbose);
                     ...
                     bool                                  a_computeRelativeError)
{
 ...
 #ifdef CH_MPI
       COMPARE_CODE_NOT_WRITTEN_FOR_PARALLEL
         (due to nested data iterators)
         MayDay::Error("Compare code not written for parallel");
 #endif
       for (crseExactDit.begin(); crseExactDit.ok(); ++crseExactDit)
       {
         if (a_exactDx < a_computedDx)
         {
           ...
           {
             Box coarseBox(coarsenedFineGrids.get(crseExactDit()));
             ...
             if (!coarseBox.isEmpty())
             {
               ...
               LapBox &= fineBox;
               fineTemp.setVal(0.0);
               FORT_AVERAGEHO(CHF_FRA1(averagedExact[crseExactDit()], comp),
         ...
         else
         {
           ...
           averagedExact[crseExactDit()].copy(exactSolnRef[ditFine()]);
         }
       } // end loop over coarsened fine boxes
