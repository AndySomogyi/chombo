...
template<int DIM> class Bocks
{
  public:
    ...
    Bocks( IndexTM<int,DIM> const & lo, IndexTM<int,DIM> const & hi )
      : m_smallEnd(lo), m_bigEnd(hi) { }
    Bocks( Bocks const & that ) : m_smallEnd( that.m_smallEnd ),
                                  m_bigEnd( that.m_bigEnd ) { }
    Bocks( Box const & chomboBox ) : m_smallEnd( chomboBox.smallEnd().dataPtr()),
    ...
    IndexTM<int,DIM> const & smallEnd() const { return m_smallEnd; }
    IndexTM<int,DIM> const & bigEnd()   const { return m_bigEnd; }
    IndexTM<int,DIM> & smallEnd()  { return m_smallEnd; }
    IndexTM<int,DIM> & bigEnd()    { return m_bigEnd; }
...
{
    int buf1[] = {1,1,1,1,1,1};
    IndexTM<int,DIM> iv1( buf1 );

    int buf2[] = {3,4,5,6,7,8};
