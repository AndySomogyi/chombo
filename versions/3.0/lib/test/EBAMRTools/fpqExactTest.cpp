                 ...
                 const Box& a_domainCoar)
{
  ...
  for(LayoutIterator lit = a_gridsFine.layoutIterator();
      lit.ok(); ++lit)
    {
      Box thisBox = a_gridsFine.get(lit());
      thisBox.grow(1);
      thisBox &= a_domainFine;
      blNormFine.ref(lit()) = thisBox;
    }
  blNormFine.close();
  for(LayoutIterator lit = a_gridsCoar.layoutIterator();
      lit.ok(); ++lit)
    {
      Box thisBox = a_gridsCoar.get(lit());
      thisBox.grow(1);
      thisBox &= a_domainCoar;
      blNormCoar.ref(lit()) = thisBox;
    }
  blNormCoar.close();
             ...
             const Real& a_dxFine)
{
  ...
  LevelData<EBCellFAB> phiCoarOld(gridsCoar, nvar,
                                  IntVect::Unit,ebcellfactCoar);
  LevelData<EBCellFAB> phiCoarNew(gridsCoar, nvar,
                                  IntVect::Unit,ebcellfactCoar);
  ...
  EBPWQuadFillPatch interpOp(a_gridsFine, gridsCoar,
