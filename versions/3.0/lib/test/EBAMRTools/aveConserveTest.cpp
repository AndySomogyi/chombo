      ...
      for(VoFIterator vofit(ivsIter, ebisBox.getEBGraph()); vofit.ok(); ++vofit)
        {
          ...
          if(a_doingRZ)
            {
              Real compVol;
              EBArith::getCompVolRZ(compVol, ebisBox, a_dx, vof, false);
            }
          else
            {
                     ...
                     bool                             a_doingRZ)
{
  ...
  if(!a_doingRZ)
    ...
    {
      //not possible to make rz perfectly conservative
      pout() << "rz sum before averaging = " << sumBeforeAverage << endl;
      pout() << "rz sum  after averaging = " << sumAfterAverage << endl;
    }
...
main(int argc, char** argv)
{
  ...
  //begin forever present scoping trick
  {
    ...
    int idorz;
    pp.get("rz_coords", idorz);
    bool doRZ = (idorz==1) && (SpaceDim==2);
    eekflag = testConservation(grids, domainCoarsest, refRat, dxCoar, doRZ);
    if(eekflag != 0)
      {
