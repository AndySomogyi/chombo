  ...
  for(int ilev = 0; ilev < amrLevelData.size(); ilev++)
    {
      ...
      a_datum[ilev] = new LevelData<EBCellFAB>();
      amrGodunovData->fillConsAndPrim(*a_datum[ilev]);
    }
}
/***************/
void
...
solutionErrorTest(int testverbosity, int fileout)
{
  ...
  generateData(solutFine, gridsFine, ebislFine, boxesFine, refRat, domaiFine, refFine);

  if(testverbosity >= 1) pout() << "generating medi solution" << endl;
  generateData(solutMedi, gridsMedi, ebislMedi, boxesMedi, refRat, domaiMedi, refMedi);

  if(testverbosity >= 1) pout() << "generating coar solution" << endl;
  generateData(solutCoar, gridsCoar, ebislCoar, boxesCoar, refRat, domaiCoar, refCoar);
