...
{
  CH_TIME("EBPatchPolytropic::getRHSTemperature");
  a_rhsTemp.setVal(0.);
  //increment by each direction
  BaseFab<Real>& regRHSTemp = a_rhsTemp.getSingleValuedFAB();
  for(int idir = 0; idir < SpaceDim; idir++)
    {
      ///the u lapl u term must get added in later (when we have u at the proper time centering)
      /**/
      BaseFab<Real>& regPrimTemp = a_facePrim[idir].getSingleValuedFAB();
      FORT_INCRRHSTEMPERATURE(CHF_FRA1(regRHSTemp, 0),
                              CHF_CONST_FRA(regPrimTemp),
                              CHF_CONST_REAL(a_specHeat),
          ...
          for(int diffDir = 0; diffDir < SpaceDim; diffDir++)
            {
              ...
              Real tempHi, tempLo;
              FORT_POINTGETTEMPFROMPRIM(CHF_REAL(tempLo), CHF_VR(primLo), CHF_CONST_REAL(a_specHeat));
              FORT_POINTGETTEMPFROMPRIM(CHF_REAL(tempHi), CHF_VR(primHi), CHF_CONST_REAL(a_specHeat));

              int dirc = QVELX + diffDir;
              Real rhoave   =   0.5*(primHi[QRHO ] +  primLo[QRHO ]);
              Real ufaceave =   0.5*(primHi[dirc ] +  primLo[dirc ]);
              Real pave     =   0.5*(primHi[QPRES] +  primLo[QPRES]);
              Real udiff    =       (primHi[dirc ] -  primLo[dirc ]);
              Real ediff =      a_specHeat*(tempHi - tempLo);

              Real pDivU = pave*udiff/m_dx[diffDir];
              Real rhoUDele = rhoave*ufaceave*ediff/m_dx[diffDir];
              ///the u lapl u term must get added in later (when we have u at the proper time centering)
              irregRHSTemp += -pDivU - rhoUDele;

            }
          a_rhsTemp(vof, 0) = irregRHSTemp;
        }
...
void
EBPatchPolytropic::
getRHSVelocity   (EBCellFAB&          a_rhsVelo,
                  EBFluxFAB&          a_facePrim,
                  BaseIVFAB<Real>     a_coveredPrimLo[SpaceDim],
                  BaseIVFAB<Real>     a_coveredPrimHi[SpaceDim],
                  const Box&          a_box,
                  const IntVectSet&   a_ivsSmall)
{
  CH_TIME("EBPatchPolytropic::getRHSVelocity");

  //this is necessary since we are doing increments from here
  a_rhsVelo.setVal(0.); 
  BaseFab<Real>& regRHS = a_rhsVelo.getSingleValuedFAB();
  for(int varDir = 0; varDir < SpaceDim; varDir++)
    {
      for(int faceDir = 0; faceDir < SpaceDim; faceDir++)
        {
          const BaseFab<Real>& regPrim = a_facePrim[faceDir].getSingleValuedFAB();
          FORT_INCRRHSVELOCITY(CHF_FRA1(regRHS, varDir),
                               CHF_CONST_FRA(regPrim),
                               CHF_CONST_REAL(m_dx[faceDir]),
                               CHF_CONST_INT(faceDir),
                               CHF_CONST_INT(varDir),
                               CHF_BOX(a_box));
        }
    }
                ...
                for(int diffDir = 0; diffDir < SpaceDim; diffDir++)
                  {
                    ...
                    fillValues(primHi, facesHi, a_facePrim[diffDir], a_coveredPrimHi[diffDir], vof);
                    int dirc = QVELX + diffDir;
                    int varc = QVELX + varDir;
                    Real rhoave   =   0.5*(primHi[QRHO ] +  primLo[QRHO ]);
                    Real ufaceave =   0.5*(primHi[dirc ] +  primLo[dirc ]);
                    Real uvardiff =       (primHi[varc ] -  primLo[varc ]);
                    Real pdiff    =       (primHi[QPRES] -  primLo[QPRES]);

                    Real rhoUdelU = rhoave*ufaceave*uvardiff/m_dx[diffDir];
                    Real gradP = 0;
                    if(varDir == diffDir)
                      {
                        gradP = pdiff/m_dx[varDir];
                      }
                    
                    irregRHSVelo  += -rhoUdelU - gradP;
                  }
                a_rhsVelo(vof, varDir) = irregRHSVelo;
              }
          }
    }//end loop over irreg vofs
}
