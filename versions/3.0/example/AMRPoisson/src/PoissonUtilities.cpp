             ...
             const PoissonParameters&                     a_params)
{
  ...
  pp2.query("s_maxCoarse", AMRPoissonOp::s_maxCoarse);
  ...
  a_solver.define(coarsestDomain, castFact,  &a_bottomSolver, a_params.numLevels);
  ...
  pp2.get("hang",      hang);
  pp2.query("m_pre", a_solver.m_pre);
  pp2.query("m_post", a_solver.m_post);
  pp2.query("m_bottom", a_solver.m_bottom);
  ...
  for(int ilev = 0; ilev < a_params.numLevels; ilev++)
    {
      ...
      setViscousCoefs(*a_eta[ilev], *a_lambda[ilev], dxLev, a_params);
      dxLev /=      a_params.refRatio[ilev];
  }
}
/****/
                          ...
                          const PoissonParameters&                     a_params)
{
  ...
  defineViscousTensorCoef(eta, lambda,  a_grids, a_params);
  int nlevels = a_grids.size();
  Vector<RefCountedPtr<LevelData<FArrayBox> > >      acoef(nlevels);
  Real acoefVal;
  pp2.get("acoef_value", acoefVal);
  Real dxLev = a_params.coarsestDx;
  for(int ilev = 0; ilev < nlevels; ilev++)
    {
      ...
      for(DataIterator dit = a_grids[ilev].dataIterator(); dit.ok(); ++dit)
        {
          (*acoef[ilev])[dit()].setVal(acoefVal);
          locGetAcoef(*acoef[ilev], a_grids[ilev], dxLev);
        }
      dxLev *= a_params.refRatio[ilev];
    }

  ViscousTensorOpFactory opFactory(a_grids, eta, lambda, acoef, 
...
void setKLBResistive(LevelData<FArrayBox>&    a_klb,
                     ...
                     const PoissonParameters& a_params)
{
  //always three comps in MHD land
  CH_assert(a_klb.nComp() == 3);
