...
void setExactVel(LevelData<FluxBox>&      a_vel,
                 const Real&              a_dx,
              ...
              const PoissonParameters&             a_params)
{
  ...
  Vector<LevelData<FluxBox>*   >  velocity(nlevels, NULL);
  Vector<LevelData<FluxBox>*   >  gradient(nlevels, NULL);
    ...
    for(int ilev = 0; ilev < nlevels; ilev++)
      {
        ...
        velocity[ilev] = new LevelData< FluxBox >(a_grids[ilev],  1,  IntVect::Zero);
        gradient[ilev] = new LevelData< FluxBox >(a_grids[ilev],  1,  IntVect::Zero);
  ...
  //create error = divergence of the velocity field after projection.
  {
    ...
    for(int ilev = 0; ilev < nlevels; ilev++)
      {
        LevelMACProjector macProj(a_grids[ilev], domLev, solver, dxLev, ilev, nlevels,
        ...
        macProj.project(*velocity[ilev],
                        *gradient[ilev],
                        *pressure[ilev],
                        presCoar);

        macDivergence(*a_error [ilev],
                      *velocity[ilev],
                      a_grids  [ilev],
                      domLev, dxLev);
