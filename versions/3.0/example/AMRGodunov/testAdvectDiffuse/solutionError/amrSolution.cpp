  ...
  for(int ilev = 0; ilev < amrLevelData.size(); ilev++)
    {
      ...
      LevelData<FArrayBox>& amrData = amrGodunovData->getStateNew();
      a_grids[ilev] = amrData.disjointBoxLayout();
      a_datum[ilev] = new LevelData<FArrayBox>(a_grids[ilev], amrData.nComp(), IntVect::Zero);
      Interval interv(0, amrData.nComp()-1);
      amrData.copyTo(interv, *a_datum[ilev], interv);
    }
}
...
solutionErrorTest(int testverbosity, int fileout)
{
  ...
  generateData(solutFine, gridsFine, boxesFine, refRat, domaiFine, refFine);

  if(testverbosity >= 1) pout() << "generating medi solution" << endl;
  generateData(solutMedi, gridsMedi, boxesMedi, refRat, domaiMedi, refMedi);

  if(testverbosity >= 1) pout() << "generating coar solution" << endl;
  generateData(solutCoar, gridsCoar, boxesCoar, refRat, domaiCoar, refCoar);
