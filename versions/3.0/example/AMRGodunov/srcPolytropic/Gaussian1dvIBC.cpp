...
#include "Gaussian1dvIBCF_F.H"

// Null constructor
Gaussian1dvIBC::Gaussian1dvIBC()
{
  m_isFortranCommonSet = false;
}

// Constructor which defines parameters used by Fortran routines
Gaussian1dvIBC::Gaussian1dvIBC(Real&           a_smallPressure,
                         const Real&     a_gamma,
...
void Gaussian1dvIBC::setFortranCommon(Real&           a_smallPressure,
                                   const Real&     a_gamma,
                                   const Real&     a_ambientDensity,
                                   const Real&     a_deltaDensity,
                                   const int&      a_pressure,
                                   ...
                                   const Real&     a_artvisc)
{
  ...
  FORT_GAUSSIAN1DVSETF(CHF_REAL(a_smallPressure),
...
PhysIBC* Gaussian1dvIBC::new_physIBC()
{
  Gaussian1dvIBC* retval = new Gaussian1dvIBC();
...
void Gaussian1dvIBC::initialize(LevelData<FArrayBox>& a_U)
{
  ...
  for (dit.begin(); dit.ok(); ++dit)
  {
    ...
    FORT_GAUSSIAN1DVINITF(CHF_FRA(U),
                       CHF_CONST_REAL(m_dx),
                       CHF_BOX(uBox));
  }
  // petermc, 9 Sep 2008:  goes with averaging
  a_U.exchange();
}

// Set boundary fluxes
void Gaussian1dvIBC::primBC(FArrayBox&            a_WGdnv,
...
void Gaussian1dvIBC::setBdrySlopes(FArrayBox&       a_dW,
                                const FArrayBox& a_W,
                                const int&       a_dir,
                                const Real&      a_time)
{
...
void Gaussian1dvIBC::artViscBC(FArrayBox&       a_F,
                            const FArrayBox& a_U,
