...
#include "Gaussian1dIBCF_F.H"

// Null constructor
Gaussian1dIBC::Gaussian1dIBC()
{
  m_isFortranCommonSet = false;
}

// Constructor which defines parameters used by Fortran routines
Gaussian1dIBC::Gaussian1dIBC(Real&           a_smallPressure,
                         const Real&     a_gamma,
...
void Gaussian1dIBC::setFortranCommon(Real&           a_smallPressure,
                                   const Real&     a_gamma,
                                   const Real&     a_ambientDensity,
                                   const Real&     a_deltaDensity,
                                   const int&      a_pressure,
                                   ...
                                   const Real&     a_artvisc)
{
  ...
  FORT_GAUSSIAN1DSETF(CHF_REAL(a_smallPressure),
...
PhysIBC* Gaussian1dIBC::new_physIBC()
{
  Gaussian1dIBC* retval = new Gaussian1dIBC();
...
void Gaussian1dIBC::initialize(LevelData<FArrayBox>& a_U)
{
  ...
  for (dit.begin(); dit.ok(); ++dit)
  {
    ...
    FORT_GAUSSIAN1DINITF(CHF_FRA(U),
                       CHF_CONST_REAL(m_dx),
                       CHF_BOX(uBox));
  }
...
// Set boundary fluxes
void Gaussian1dIBC::primBC(FArrayBox&            a_WGdnv,
...
void Gaussian1dIBC::setBdrySlopes(FArrayBox&       a_dW,
                                const FArrayBox& a_W,
                                const int&       a_dir,
                                const Real&      a_time)
{
...
void Gaussian1dIBC::artViscBC(FArrayBox&       a_F,
                            const FArrayBox& a_U,
