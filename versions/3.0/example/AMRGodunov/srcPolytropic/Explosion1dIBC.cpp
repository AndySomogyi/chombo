...
void
Explosion1dIBC::setFortranCommon(Real&           a_smallPressure,
                               const Real&     a_gamma,
                               const Real&     a_ms,
                               const RealVect& a_center,
                               const Real&     a_size,
                               ...
                               const Real&     a_artvisc)
{
  ...
  FORT_EXPLOSION1DSETF(CHF_REAL(a_smallPressure),
...
PhysIBC* Explosion1dIBC::new_physIBC()
{
  Explosion1dIBC* retval = new Explosion1dIBC();
...
void Explosion1dIBC::initialize(LevelData<FArrayBox>& a_U)
{
  ...
  for (dit.begin(); dit.ok(); ++dit)
  {
    ...
    FORT_EXPLOSION1DINITF(CHF_FRA(U),
                        CHF_CONST_REAL(m_dx),
                        CHF_BOX(uBox));
  }
}
...
void Explosion1dIBC::artViscBC(FArrayBox&       a_F,
                             const FArrayBox& a_U,
