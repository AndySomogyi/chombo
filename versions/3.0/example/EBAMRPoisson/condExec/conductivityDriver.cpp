...
void solve(const PoissonParameters&  a_params)
{
  ...
  defineConductivitySolver(solver, grids, ebisl, newBottomSolver, a_params);
...
//       UnfreedMemory();
      //output the answer
      char charstr[100];
      sprintf(charstr, "vel.%dd.hdf5", 1);
      string filename(charstr);
      Vector<string> names(1, string("vel"));
      bool replaceCovered;
      ...
      char charstrrhs[100];
      sprintf(charstrrhs, "rhs.%dd.hdf5", 1);
      string filenamerhs(charstrrhs);
      Vector<string> namesrhs(1, string("rhs"));
