  ...
  for (int ilev = 0; ilev < nlev; ilev++)
    {
      ...
      ebisPtr->fillEBISLayout(a_ebislFine[ilev], a_gridsFine[ilev], domLevFine.domainBox(), numEBG);
      ebisPtr->fillEBISLayout(a_ebislMedi[ilev], a_gridsMedi[ilev], domLevMedi.domainBox(), numEBG);
      ebisPtr->fillEBISLayout(a_ebislCoar[ilev], a_gridsCoar[ilev], domLevCoar.domainBox(), numEBG);
               ...
               int a_whichGeomForced)
{
  ...
  Box finestDomain = a_coarsestDomain.domainBox();
      ...
      if (whichgeom == 0)
        {
          ...
          ebisPtr->define(finestDomain, origin, fineDx, regserv, ebMaxSize);
        }
      else if (whichgeom == 1)
        {
          ...
          ebisPtr->define(finestDomain, origin, fineDx, workshop, ebMaxSize);
        }
      else if (whichgeom == 4)
        {
...
//           bool negativeInside = true;
      ...
      else if (whichgeom == 2)
        {
          ...
          //this generates the new EBIS
          RealVect origin = RealVect::Zero;
          ebisPtr->define(finestDomain, origin, fineDx, slab, ebMaxSize);
        }
      else if (whichgeom == 5)
        {
    ...
    vectDx *= fineDx;
          ...
          ebisPtr->define(finestDomain,origin,fineDx,geometryshop,ebMaxSize);
        }
    ...
    vectDx *= fineDx;
      ...
      else if (whichgeom == 18)
        {
          ...
          // This generates the new EBIS
          EBIndexSpace* ebisPtr = Chombo_EBIS::instance();
          ebisPtr->define(finestDomain, origin, fineDx, workshop);

        }
      else if (whichgeom == 19)
        {
          pout() << "sphere inside a tilted cylinder geometry " << endl;
          vector<Real> sphere_center(SpaceDim);
          pp.getarr("sphere_center",sphere_center, 0, SpaceDim);
          RealVect sphereCenter;
          for (int idir = 0; idir < SpaceDim; idir++)
          ...
          SphereIF*          sphere = (new  SphereIF         (sphereRadius, sphereCenter, fluidInside)                       );
          TiltedCylinderIF*  tunnel = (new  TiltedCylinderIF (cylinderRadius, cylinderAxis, cylinderCenterPt, cylinderInside));

          IntersectionIF intersect(*tunnel, *sphere);

          RealVect vectDx = RealVect::Unit;
          vectDx *= fineDx;

          GeometryShop workshop(intersect,verbosity,vectDx);
          //this generates the new EBIS
          pout() << "ebisPtr->define before" << endl;
          ebisPtr->define(finestDomain, origin, fineDx, workshop, ebMaxSize);
          pout() << "ebisPtr->define after" << endl;
      ...
      else if (whichgeom == 20)
        {
          ...
          Real minX = origin[0];
          Real maxX = a_params.m_domainLength;

          ArteryIF arteryIF(arteryType,arteryRadius,arteryPerturb,minX,maxX,arteryCenter,arteryInside);
