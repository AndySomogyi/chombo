                 ...
                 const Vector<Vector<Box> > &               a_vectBoxes)
{
  ...
  Real tubeRadius, maxVel;
  Vector<Real> centerPtVect, tubeAxisVect;
  pp.get("max_vel",     maxVel);
  pp.get("cylinder_radius", tubeRadius);
  pp.getarr("cylinder_center_pt",  centerPtVect,  0,SpaceDim);
  pp.getarr("cylinder_axis",       tubeAxisVect,0,SpaceDim);
  ...
  Real sum;
  PolyGeom::unifyVector(tubeAxis, sum);

  Real viscosity = 0.0;
  pp.get("viscosity", viscosity);

  PoisselleTubeFactory ibc(centerPt, tubeAxis, tubeRadius, maxVel);
              ...
              bool a_atFinestLevel)
{
  ...
  ParmParse pp;
  pp.get("num_init_iterations", numInitIterations);

  pout() << "generating fine soln " << endl;
  int initIter = numInitIterations;
  getSolution(solnFine, a_dt, gridsFine, ebislFine, a_domainFine, a_names, a_params,  initIter, a_atFinestLevel, a_vectBoxesFine);

  pout() << "generating coarse soln" << endl;
  initIter = numInitIterations;
  getSolution(solnCoar, a_dt, a_gridsCoar, a_ebislCoar, a_domainCoar, a_names, a_params,  initIter, false, a_vectBoxesCoar);

  pout() << "generating truncation error = (Ave(fine soln) - coarse soln)/dt" << endl;
...
int main(int argc, char* argv[])
{
  ...
  // Scoping trick
  {
    ...
    //4 -> forcing cylindrical geom
    AMRINSGeometry(params, domainFine, 4);

    Vector<Vector<Box> > boxesFine, boxesMedi, boxesCoar;
    getFixedGrids(boxesFine, boxesMedi, boxesCoar, params, domainCoar, true);
