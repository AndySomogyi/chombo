...
/****/
Real getVelocity(const RealVect& a_xval, int a_velDir)
{
...
/****/
void
setExactVeloc( EBCellFAB           &  a_cellVeloc,
               const Box           &  a_grid,
  ...
  for(VoFIterator vofit(ivsGrid, a_ebisBox.getEBGraph()); vofit.ok(); ++vofit)
    {
      ...
      for(int faceDir = 0; faceDir < SpaceDim; faceDir++)
        {
          a_cellVeloc(vof, faceDir) = getVelocity(xvalVoF, faceDir);
        }
    }
}
...
/****/
void
setExactVeloc(Vector< LevelData<EBCellFAB>* > &  a_cellVeloc,
  ...
  for(int ilev = 0; ilev < a_nlevels; ilev++)
    {
      for(DataIterator dit = a_grids[ilev].dataIterator(); dit.ok(); ++dit)
        {
          setExactVeloc((*a_cellVeloc[ilev])[dit()],
...
/****/
/****/
void getAdvectionVel(Vector< LevelData<EBFluxFAB>* >&           a_macAdvVel,
                     Real&                                      a_dt,
                     ...
                     const Vector<Vector<Box> > &               a_vectBoxes)
{
  ...
  pout() << "doing inviscid advective velocity calc" << endl;
  bool doSlip = false;
  int orderEBBC =2;
  ...
  int nlevels = a_params.m_maxLevel + 1;
  a_macAdvVel.resize(nlevels, NULL);
  ...
  for(int ilev = 0; ilev < nlevels; ilev++)
    {
      ...
      a_macAdvVel[ilev] = new LevelData<EBFluxFAB>(a_grids[ilev],        1,  ivghost, ebfluxfact);
      ...
      EBLevelDataOps::setVal(*a_macAdvVel[ilev], 0.0);
  ...
  pout() << "computing advection velocity" << endl;
  ...
  amr.allocateTemporaries();
  amr.computeAdvectionVelocities(a_macAdvVel,
                                 coveredAdvVelLo,
                                 coveredAdvVelHi,
                                 cellVeloc);
  amr.deleteTemporaries();
...
int main(int argc, char* argv[])
{
  ...
  // Scoping trick
  {
    ...
    pout() << "generating fine advection velocity " << endl;
    Real dt;
    getAdvectionVel(scalFine, dt, gridsFine, ebislFine, domainFine, params, true,   fineBoxes);

    pout() << "generating medium advection velocity and error" << endl;
    getAdvectionVel(scalMedi, dt, gridsMedi, ebislMedi, domainMedi, params, false,  mediBoxes);
    ...
    pout() << "generating coarse advection velocity and error" << endl;
    getAdvectionVel(scalCoar, dt, gridsCoar, ebislCoar, domainCoar, params, false, coarBoxes);
