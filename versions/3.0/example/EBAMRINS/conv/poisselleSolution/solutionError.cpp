                 ...
                 const Vector<Vector<Box> > &               a_vectBoxes)
{
  ...
  //want the stopping criteria to be the number of steps
...
int main(int argc, char* argv[])
{
  ...
  // Scoping trick
  {
    ...
    params.m_initIterations = numInitIterations;
    ...
    if(pp.contains("which_geom"))
    ...
    else
      {
        ...
        getFixedGrids(fineBoxes, mediBoxes, coarBoxes, params, domainCoar, 4);
      }
...
#if CH_SPACEDIM==2
    string fileFine("fineError.2d.hdf5");
    string fileCoar("coarError.2d.hdf5");
#else
    string fileFine("fineError.3d.hdf5");
    string fileCoar("coarError.3d.hdf5");
#endif
    int dofileout;
