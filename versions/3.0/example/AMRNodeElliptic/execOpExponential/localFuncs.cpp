...
// --------------------------------------------------------------
int
setLaplacianExact(Vector<LevelData<NodeFArrayBox>* >& a_vectRhs,
      ...
      for (dit.reset(); dit.ok(); ++dit)
        {
          ...
          /**/
          FORT_GETPHINODELAP(CHF_FRA(rhsFab),
                             CHF_BOX(rhsFab.box()),
...
// --------------------------------------------------------------
int
setPhiExp(Vector<LevelData<NodeFArrayBox>* >& a_phi,
          const Vector<Real>&                 a_vectDx,
          const Vector<ProblemDomain>&        a_vectDomain,
          const int a_numlevels,
          const bool a_verbose)
      ...
      for (DataIterator it(phiThisLev->dataIterator()); it.ok(); ++it)
        {
          ...
          FORT_GETPHINODEEXP(CHF_FRA(phiFab),
                             CHF_BOX(phiFab.box()),
                             CHF_CONST_REAL(dxlev));
        }
    }
