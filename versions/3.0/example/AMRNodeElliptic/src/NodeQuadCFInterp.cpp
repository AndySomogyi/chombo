...
// NodeQuadCFInterp.cpp
// petermc, Mon, Apr 23, 2001
// petermc, 1 Nov 2001, redid this so that it uses NodeQuadCFInterp2
#include "NodeQuadCFInterp.H"

// ---------------------------------------------------------
void
NodeQuadCFInterp::setDefaultValues()
{
  m_isDefined = false;
  m_isBCDefined = false;
  m_verbose = false;
                                   ...
                                   bool a_verbose)
{
  ...
  define(a_grids, a_dx, probdomain,
         a_loCFIVS, a_hiCFIVS,
         a_refToCoarse, a_interpolationDegree, a_ncomp, a_verbose);
}
...
// ---------------------------------------------------------
bool
NodeQuadCFInterp::isDefined() const
{
  return(m_isDefined);
}
...
// ---------------------------------------------------------
void
NodeQuadCFInterp::define(const DisjointBoxLayout& a_grids,
                         Real a_dx,
                         const Box& a_domain,
                         const LayoutData<NodeCFIVS>* const a_loCFIVS,
                         const LayoutData<NodeCFIVS>* const a_hiCFIVS,
...
// ---------------------------------------------------------
void
NodeQuadCFInterp::setDomainNodeBC(const DomainNodeBC& a_dombcIn)
{
  m_dombc = a_dombcIn;
  m_isBCDefined = true;
}
...
// ---------------------------------------------------------
void
NodeQuadCFInterp::setVerbose(bool a_verbose)
{
  m_verbose = a_verbose;
}
...
// ---------------------------------------------------------
void
NodeQuadCFInterp::coarseFineInterp(LevelData<NodeFArrayBox>& a_phiFine,
                                   ...
                                   bool a_inhomogeneous)
{
  ...
  if (m_coarsenings == 1)
  ...
  else // m_coarsenings >= 2
    {
      ...
      // Set physical boundary conditions on *m_inter[0].  Need m_dombc.
      ...
      if (a_inhomogeneous)
        for (DataIterator dit = m_inter[0]->dataIterator(); dit.ok(); ++dit)
          m_dombc.applyInhomogeneousBCs(m_inter[0]->operator[](dit()), domain, dx);
      else
        for (DataIterator dit = m_inter[0]->dataIterator(); dit.ok(); ++dit)
          m_dombc.applyHomogeneousBCs(m_inter[0]->operator[](dit()), domain, dx);
      ...
      for (int interlev = 1; interlev < m_coarsenings - 1; interlev++)
        {
          ...
          dx *= 0.5;
          if (a_inhomogeneous)
            for (DataIterator dit = m_inter[interlev]->dataIterator(); dit.ok(); ++dit)
              m_dombc.applyInhomogeneousBCs(m_inter[interlev]->operator[](dit()), domain, dx);
          else
            for (DataIterator dit = m_inter[interlev]->dataIterator(); dit.ok(); ++dit)
              m_dombc.applyHomogeneousBCs(m_inter[interlev]->operator[](dit()), domain, dx);
        }
