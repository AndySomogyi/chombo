...
// --------------------------------------------------------------
int
setGradExact(Vector<LevelData<NodeFArrayBox>* >& a_grad,
             const Vector<Real>&                 a_vectDx,
             const Vector<ProblemDomain>&        a_vectDomain,
             const int a_numlevels,
             const bool a_verbose)
  ...
  for (int ilev = 0; ilev < a_numlevels; ilev++)
    {
      ...
      if (a_verbose)
        cout << "level " << ilev << " dx = " << dxlev << endl;
      LevelData<NodeFArrayBox>* gradThisLev = a_grad[ilev];
      for (DataIterator it(gradThisLev->dataIterator()); it.ok(); ++it)
        {
          FArrayBox& gradFab = gradThisLev->operator[](it()).getFab();
          // gradFab.box() is node-centered.

          FORT_GETGRADNODE(CHF_FRA(gradFab),
                           CHF_BOX(gradFab.box()),
                           CHF_CONST_REAL(dxlev));

          // dummy statement in order to be able to get around gdb bug
          if (false)CH_assert(0 == 0);
        }
    }
  return(0);
}
