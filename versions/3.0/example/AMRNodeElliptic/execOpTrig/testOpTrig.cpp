...
/**
   Computes
   L(phi)
   where phi is a periodic, trigonometric function
...
int main(int argc, char* argv[])
{
  ...
  //scoping trick
  {
    ...
    center *= 0.5;
    RealVect lengths = (probLo - probHi);

    FORT_INITTRIG(CHF_REALVECT(center),
                  CHF_REALVECT(lengths));
    ...
    eekflag = setPhiTrig(phi, vectDx, vectDomain, numlevels, verbose);
    if (verbose)
      cout << "Did setPhiTrig on phi" << endl;
    if (eekflag != 0)
      {
        cerr << "error: setPhiTrig returned error code "
             << eekflag << endl;
        exit(4);
      }
    ...
    eekflag = setPhiTrig(phi2, vectDx2, vectDomain2, numlevels, verbose);
    if (verbose)
      cout << "Did setPhiTrig on phi2" << endl;
    if (eekflag != 0)
      {
        cerr << "error: setPhiTrig returned error code "
             << eekflag << endl;
    ...
    */
    ...
    outvars.push_back(diff); outnames.push_back(string("L(phi)_-_refined L(phi)"));
